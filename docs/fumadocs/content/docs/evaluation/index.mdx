...
For a first end-to-end path, use [Quickstart](/docs/quickstart). Ten catalog-backed generate-then-score cells (no invented ranks) live in [Model → benchmark recipes](/docs/evaluation/model-benchmark-recipes). For flags and discovery commands, see [CLI](/docs/reference/cli). For per-benchmark layouts, see [Benchmark Hub](/docs/evaluation/benchmark-hub). For metric recipes, see [Metrics](/docs/evaluation/metrics).
...
- **Reproduce a known model × benchmark configuration** — `worldfoundry-eval reproduce --recipe <recipe.yaml> ...`
...
- **Official reproduction** — Pinned recipe, official sample coverage, model generation settings, and benchmark runtime settings. Classification: `reproduction`.
- **Model benchmark** — In-tree model plus the benchmark's official prompt/data protocol. Classification: `protocol_compliant`.
- **Score my artifacts** — User-provided artifacts plus a benchmark's complete metric protocol. Classification: `custom_dataset_metric_evaluation`.
- **Generate and score my dataset** — Dataset manifest plus selected executable metric IDs. Classification: `custom_dataset_metric_evaluation`.
- **Score existing results** — WorldFoundry JSON/JSONL results plus selected executable metric IDs. Classification: `custom_results_metric_evaluation`.
...
Use `--recipe` for an immutable model × benchmark cell. A reproduction recipe keeps upstream model and benchmark settings separate, so benchmark parameters cannot silently override orchestrator-owned paths or modes. Scope and eligibility are part of the prepared plan rather than inferred from a successful process exit.
...
- [Model → benchmark recipes](/docs/evaluation/model-benchmark-recipes) — ten catalog-backed inference/eval cells
