...
| `WORLDFOUNDRY_REALTIME_FRAME_QUEUE_POLICY` | `latest-interactive` | Use `latest-interactive` to bound latency under congestion or `ordered-quality` to preserve every frame. Count-exact prompt/segment sessions always enforce `ordered-quality`. |
| `WORLDFOUNDRY_REALTIME_PRESENTATION_MODE` | `hold-last` | WebRTC A/B switch. `hold-last` preserves the compatible constant clock; `real-frames` gives pacing to the chunk presentation manager and sends no repeated frames through its two-frame mailbox. |
| `WORLDFOUNDRY_REALTIME_PRESENTATION_MAX_FPS` | `2 × model FPS` | Maximum active-chunk drain rate while a pending chunk is waiting in `real-frames` mode. |
| `WORLDFOUNDRY_REALTIME_SOCKET_PACING_MODE` | `legacy-dual` | WebSocket A/B switch. `client-only` disables server-side frame sleeps and leaves the existing browser presentation clock as the sole pacing owner. |
...
| `WORLDFOUNDRY_REALTIME_PREWARM_TIMEOUT_SECONDS` | unset | Optional deadline for configure plus warmup chunks. The server reports an overrun after the current non-cancellable model call returns; `0` disables it. |
| `WORLDFOUNDRY_REALTIME_POSTPROCESS_PRESET` | `identity` | Select `identity`, `rtx-super-resolution`, `rtx-super-resolution-ultra`, or `rtx-deblur-ultra`. RTX choices are explicit opt-ins and fail before serving if their runtime is unavailable. |
| `WORLDFOUNDRY_REALTIME_RTX_SCALE` | preset default (`2`, or `1` for deblur) | Spatial VSR scale. NVIDIA VFX supports up to 4x; same-resolution denoise/deblur modes require `1`. |
| `WORLDFOUNDRY_REALTIME_RTX_OUTPUT_WIDTH` / `WORLDFOUNDRY_REALTIME_RTX_OUTPUT_HEIGHT` | unset | Optional explicit RTX output dimensions. Set both or neither. |
| `WORLDFOUNDRY_REALTIME_RTX_QUALITY` | preset default | Override the NVIDIA VFX quality enum, for example `HIGH`, `ULTRA`, `DEBLUR_ULTRA`, or `HIGHBITRATE_HIGH`. |
| `WORLDFOUNDRY_REALTIME_RTX_DEVICE` | index from Studio device, otherwise `0` | CUDA device index owned by the VFX effect. |
| `WORLDFOUNDRY_REALTIME_RTX_NON_BLOCKING` | `false` | Request asynchronous VFX execution. WorldFoundry still synchronizes before cloning the borrowed DLPack output, so this does not weaken memory safety. |
| `WORLDFOUNDRY_REALTIME_RTX_USE_CURRENT_STREAM` | `true` | Pass the current PyTorch CUDA stream pointer to NVIDIA VFX; disable only when diagnosing stream interop. |
| `WORLDFOUNDRY_REALTIME_PERF_LOG_INTERVAL_CHUNKS` | `5` | Emit a structured timing summary with stage p50/p90 values every N chunks; set `0` to disable periodic summaries. |
| `WORLDFOUNDRY_REALTIME_PERF_WARMUP_CHUNKS` | `0` | Mark and exclude the first N session chunks from timing summaries. Raw JSONL events retain the warmup marker. |
| `WORLDFOUNDRY_REALTIME_PERF_JSONL` | unset | Optional path for one canonical `realtime.chunk_timing` JSONL event per chunk. Use only while profiling to avoid hot-path file I/O. |
...
| `WORLDFOUNDRY_REALTIME_SHUTDOWN_TIMEOUT_SECONDS` | `5` | Total deadline for cooperative async teardown of preload, session workers, transports, and runtime reset. It bounds the shutdown handler but does not force-kill a running model thread. |
