...
| `studio/serving/realtime/` | Transport-neutral input resampling, RGB normalization, JPEG encoding, and bounded frame queues. | `RealtimeControlResampler`, `FrameQueuePolicy`, `LatestFrameBuffer`. |
| `studio/visualization/backends/world_realtime.py` | Owns the resident model thread and WebRTC/WebSocket session lifecycle. | `ResidentWorldRuntime`, `RealtimePeerManager`. |
| `core/video_postprocess.py` | Stateful, chainable output processing outside the model's encode/diffuse/decode contract. | `VideoPostprocessStream`, `VideoPostProcessorSession`. |
| `core/video_postprocess_rtx.py` | Optional layout-preserving NVIDIA VFX adapter and preflight diagnosis; imports the proprietary runtime only after explicit selection. | `RTXVideoSuperResolutionPostProcessor`, `inspect_rtx_vfx_capability()`. |
| `runtime/realtime_regression.py` | Evaluates canonical chunk-timing JSONL against explicit performance limits. | `RealtimeRegressionManifest`, `evaluate_realtime_manifest()`. |
