...
<ArchDiagram id="studio" locale="zh" />
  ...
  <section className="pi-kv-group">
    ...
    <ul>
      <li className="pi-kv-row">
        ...
        <p>从 pipeline files、canonical runtime entries 和 overrides 发现模型条目。</p>
        <p className="pi-kv-read"><span>首读</span> <code>CatalogEntry</code>、<code>discover_catalog()</code>、<code>filter_catalog()</code>、<code>find_entry()</code>、<code>catalog_stats()</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>准备输入、加载 pipeline classes、选择 runtime driver strategy、写 run records、导出 previews。</p>
        <p className="pi-kv-read"><span>首读</span> <code>PreparedInputs</code>、<code>RunRecord</code>、<code>PipelineContext</code>、<code>BaseRuntimeDriver</code>、<code>StudioManager</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p className="pi-kv-read"><span>首读</span> <code>StudioLaunchConfig</code>、<code>parse_launch_config()</code>、<code>build_demo()</code>、<code>main()</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>Theme constants 和 UI styling hooks。</p>
        <p className="pi-kv-read"><span>首读</span> 只服务 app shell。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>与 transport 无关的输入重采样、RGB 归一化、JPEG 编码和有界帧队列。</p>
        <p className="pi-kv-read"><span>首读</span> <code>RealtimeControlResampler</code>、<code>FrameQueuePolicy</code>、<code>LatestFrameBuffer</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>管理 resident model thread 以及 WebRTC/WebSocket 会话生命周期。</p>
        <p className="pi-kv-read"><span>首读</span> <code>ResidentWorldRuntime</code>、<code>RealtimePeerManager</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>位于模型 encode/diffuse/decode 契约之外的有状态链式输出处理。</p>
        <p className="pi-kv-read"><span>首读</span> <code>VideoPostprocessStream</code>、<code>VideoPostProcessorSession</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>可选、保持布局的 NVIDIA VFX adapter 与启动前诊断；只有显式选择后才 import 专有 runtime。</p>
        <p className="pi-kv-read"><span>首读</span> <code>RTXVideoSuperResolutionPostProcessor</code>、<code>inspect_rtx_vfx_capability()</code>。</p>
      </li>
      <li className="pi-kv-row">
        <div className="pi-kv-head">
          <div className="pi-kv-names">
        ...
        <p>用显式性能阈值检查规范化 chunk-timing JSONL。</p>
        <p className="pi-kv-read"><span>首读</span> <code>RealtimeRegressionManifest</code>、<code>evaluate_realtime_manifest()</code>。</p>
      </li>
    </ul>
  </section>
</div>
...
- **Launch parsing** — 留在 `app.py`：CLI flags、default UI selections、gradio launch parameters。如果满足则移到 `execution.py`：这个值影响非 UI execution。
- **HTML helpers** — 留在 `app.py`：Header、tray、cards、status blocks、summaries。如果满足则移到 `execution.py`：同样输出需要被 tests 或 CLI 使用。
- **Callback glue** — 留在 `app.py`：Button handlers 和 component updates。如果满足则移到 `execution.py`：它加载模型、触碰文件或转换 artifacts。
- **Presets** — 留在 `app.py`：UI-specific prompt/interactions defaults。如果满足则移到 `execution.py`：Preset 语义需要和 evaluation 共享。

## 实时服务边界

实时路径刻意把模型状态与展示状态分开：

<ArchDiagram id="studio-realtime" locale="zh" />

模型通过 `RealtimeSpec` 拥有因果时序；transport 不应猜测 latent stride，也不能
静默改变严格 segment 帧数。`latest-interactive` 会在有界 backpressure 后丢弃旧帧，
保护控制延迟；`ordered-quality` 从不丢帧，凡是 `chunk_done` 帧数属于播放契约的
prompt-scheduled 或 queued segment 都必须使用它。

Postprocessor 位于解码 RGB 提取之后，持有隔离的会话状态，可以暂存一个 chunk，
并按 chain 顺序 flush 尾帧。默认 identity processor 也走同一条生产边界但不改变
输出，因此未来接 VSR、去噪或插帧时不需要侵入模型专属推理循环。RTX adapter
保持 layout/container/dtype/range，每条 stream 独占一个 VFX effect，复用前先 clone
借用的 DLPack 输出，并由单独授权的 `rtx_postprocess` extra 控制。每个完成的 chunk
都会记录归一化阶段 timing；用 `runtime/realtime_regression.py` 可以对 trace 中的
p50/p90、吞吐、队列深度和掉帧做门禁。
...
面向用户的启动与 frontend 路由见 [Studio](/zh/docs/guides/studio)。Studio 作为使用界面如何把用户意图编译为与 CLI、MCP 相同的 request，见 [使用界面与编排](/zh/docs/maintainers/architecture/surfaces-orchestration)。Studio 所依赖的 pipeline / operator / `GenerationResult` 契约见 [模型运行时](/zh/docs/maintainers/architecture/model-runtime)。
