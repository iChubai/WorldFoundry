...
| `studio/serving/realtime/` | 与 transport 无关的输入重采样、RGB 归一化、JPEG 编码和有界帧队列。 | `RealtimeControlResampler`、`FrameQueuePolicy`、`LatestFrameBuffer`。 |
| `studio/visualization/backends/world_realtime.py` | 管理 resident model thread 以及 WebRTC/WebSocket 会话生命周期。 | `ResidentWorldRuntime`、`RealtimePeerManager`。 |
| `core/video_postprocess.py` | 位于模型 encode/diffuse/decode 契约之外的有状态链式输出处理。 | `VideoPostprocessStream`、`VideoPostProcessorSession`。 |
| `core/video_postprocess_rtx.py` | 可选、保持布局的 NVIDIA VFX adapter 与启动前诊断；只有显式选择后才 import 专有 runtime。 | `RTXVideoSuperResolutionPostProcessor`、`inspect_rtx_vfx_capability()`。 |
| `runtime/realtime_regression.py` | 用显式性能阈值检查规范化 chunk-timing JSONL。 | `RealtimeRegressionManifest`、`evaluate_realtime_manifest()`。 |
