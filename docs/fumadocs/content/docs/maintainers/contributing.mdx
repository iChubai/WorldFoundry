...
- **New model or runtime profile** — `guides/supported-models`, `reference/environments`, and the model guide if behavior changes. Refresh checked-in homepage recipes with `npm --prefix docs/fumadocs run models:generate`, then verify them with `npm --prefix docs/fumadocs run models:check`. To refresh paper titles and teaser figures, run `npm --prefix docs/fumadocs run models:media` after generate.
...
Training sources and local test suites are outside the public distribution. Verify the public CPU entry points with:
