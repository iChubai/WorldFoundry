...
- **新模型或 runtime profile** — `guides/supported-models`、`reference/environments`，行为变化时更新模型指南。先用 `npm --prefix docs/fumadocs run models:generate` 刷新已检入的首页 recipes，再用 `npm --prefix docs/fumadocs run models:check` 验证。论文标题与配图用 `npm --prefix docs/fumadocs run models:media` 在 generate 之后刷新。
...
公开版本不包含训练实现和本地测试套件。使用以下命令验证公开的 CPU 入口：
