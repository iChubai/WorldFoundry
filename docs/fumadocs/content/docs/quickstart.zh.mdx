...
- **安装路径：** 轻量 CPU catalog/CLI/TUI 工作使用 `python -m pip install -e ".[tui]"`；CUDA conda runtime 使用 `bash scripts/setup/bootstrap_worldfoundry.sh`。可选 extras 不会自动选择 CUDA PyTorch wheel index。
...
下面的命令描述 GPU runtime 路径。Bootstrap 会创建默认 WorldFoundry runtime，并写出一份本地 shell 文件，里面包含 CLI、Studio 和评测 runner 会用到的路径。每次打开新 shell 后都应该先 source 这个文件。
...
完整命令说明见 [CLI](/zh/docs/reference/cli)。
