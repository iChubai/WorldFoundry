...
| 复现钉死的 model × benchmark 配方 | `reproduce` | 解析 recipe YAML；若提供了命名 profile 也可解析。 |
...
| `reproduce` | 执行 recipe YAML；若存在命名 / benchmark 默认 profile 也可执行。 |
...
| `dataset` | 对 sample manifest 执行 `create`、`show`、`validate`、`materialize`；用 `locate`、`plan` 定位或规划本地/HF 数据资产。 |
...
| 标志 | 等价环境变量 | 说明 |
| --- | --- | --- |
| `--log-level` | `WORLDFOUNDRY_LOG_LEVEL` | 可选 `DEBUG`、`INFO`、`WARNING`、`ERROR` 或 `CRITICAL`；默认 `INFO`。 |
| `--log-file` | `WORLDFOUNDRY_LOG_FILE` | 可选旋转文件 sink。 |
| `--log-json` | `WORLDFOUNDRY_LOG_JSON` | 文件 sink 使用 JSON Lines；控制台仍为文本。 |
