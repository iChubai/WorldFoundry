"""LTX video VAE encoder/decoder (3-D residual UNet).

Official 8× temporal / 32× spatial compression, 128 latent
channels, BCTHW layout.  :class:`VideoEncoder` /
:class:`VideoDecoder` are the public encode/decode surfaces.
Optional spatial/temporal tiling uses :mod:`.tiling`.

:func:`map_temporal_slice` / :func:`map_spatial_slice` convert
latent tile ranges back to pixel slices with trapezoidal
blend weights.  Residual stacks are implementation detail.

Consumed by :class:`~..component.LTXMediaDecoder` and tensor
codecs; sibling of :mod:`~..audio.audio_vae`.
"""
...
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.common.normalization import PixelNorm
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.convolution import make_conv_nd
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.enums import (
    LogVarianceType,
    NormLayerType,
    PaddingModeType,
)
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.ops import (
    PerChannelStatistics,
    patchify,
    unpatchify,
)
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.resnet import ResnetBlock3D, UNetMidBlock3D
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.sampling import (
    DepthToSpaceUpsample,
    SpaceToDepthDownsample,
)
from worldfoundry.base_models.diffusion_model.models.autoencoders.ltx.video.tiling import TilingConfig
from worldfoundry.base_models.diffusion_model.models.representations.ltx.tiling import (
...
def _module_execution_target(module: nn.Module) -> tuple[torch.device, torch.dtype]:
    """Resolve the real forward target, including VRAM-managed wrappers.

    VRAM management intentionally keeps the wrapped module's source parameters
    on CPU and materializes temporary computation weights on the accelerator.
    Looking only at ``next(module.parameters())`` therefore sends inputs back to
    CPU while the wrapper executes with CUDA weights.
    """

    for child in module.modules():
        computation_device = getattr(child, "computation_device", None)
        computation_dtype = getattr(child, "computation_dtype", None)
        if computation_device is not None and computation_dtype is not None:
            return torch.device(computation_device), computation_dtype
...
class VideoEncoder(nn.Module):
    """LTX video VAE encoder: BCTHW pixels to 128-channel latents."""
    ...
    def tiled_encode(
        ...
        *,
        ...
        model_device, model_dtype = _module_execution_target(self)
...
class VideoDecoder(nn.Module):
    """LTX video VAE decoder: 128-channel latents to BCTHW pixels."""
    _DEFAULT_NORM_NUM_GROUPS = 32
    """
...
def map_temporal_slice(begin: int, end: int, left_ramp: int, right_ramp: int, scale: int) -> Tuple[slice, torch.Tensor]:
    """Map a latent time tile to a pixel slice and trapezoidal blend weights."""
    start = begin * scale
    stop = 1 + (end - 1) * scale
...
def map_spatial_slice(begin: int, end: int, left_ramp: int, right_ramp: int, scale: int) -> Tuple[slice, torch.Tensor]:
    """Map a latent spatial tile to a pixel slice and trapezoidal blend weights."""
    start = begin * scale
    stop = end * scale
