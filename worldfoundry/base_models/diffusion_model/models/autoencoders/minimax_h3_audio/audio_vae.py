...
"""MiniMax H3 DAC-lineage audio VAE encoder/decoder.

SGLang (Apache-2.0) port.  :class:`DacAudioVAE` is the CNN
graph; the public wrapper is :class:`~.minimax_h3.MiniMaxH3AudioVAE`.

Waveforms are 32 kHz mono.  Encoder rates ``[2, 4, 4, 5, 5]``
produce 32-channel / 2048-dim latents.  Causal attention and
Snake residuals are implementation detail.

Sibling of :mod:`.bigvgan`.  Shared H3 contract helpers live
in :mod:`~..minimax_h3_common`.
"""
...
class GeGluMlp(nn.Module):
    """Gated-GELU MLP used inside DAC attention projections."""
...
class CausalAttention(nn.Module):
    """Causal self-attention over DAC audio feature sequences."""
...
class AttnProjection(nn.Module):
    """Optional attention projection inserted in the DAC encoder."""
    def __init__(
...
def WNConv1d(*args, **kwargs):
    """Weight-normalized 1-D convolution constructor for DAC blocks."""
    return weight_norm(nn.Conv1d(*args, **kwargs))


@torch.jit.script
def snake(x, alpha):
    """Elementwise Snake periodic activation."""
    shape = x.shape
...
class Snake1d(nn.Module):
    """Learned Snake activation applied channel-wise to 1-D features."""
...
class ResidualUnit(nn.Module):
    """Dilated residual unit used by the DAC encoder."""
...
class EncoderBlock(nn.Module):
    """Strided DAC encoder stage wrapping residual units."""
...
class Encoder(nn.Module):
    """Full DAC convolutional encoder stack."""
    def __init__(
        self,
        d_model: int = 64,
...
class DacAudioVAE(nn.Module):
    # Ported from SGLang: the SGLang base was already ``torch.nn.Module`` (no
    # framework mixin), so no substitution was required for this class.
    """DAC-lineage audio VAE CNN used by MiniMax H3."""
    def __init__(
        self,
        encoder_dim: int = 64,
    ...
    def encode(self, audio_data: torch.Tensor, sample_rate: int = None):
        """Encode waveform into the deterministic posterior-mean latent.

        Mirrors the SGLang MiniMax H3 reference-encoding path
        (``encoder`` -> optional ``pre_block`` -> ``mean_proj``); no sampling,
        no ``latents_mean`` / ``latents_std`` normalization is applied here.

        Parameters
        ----------
        audio_data : Tensor[B x 1 x T]
            Mono waveform (per channel). Stereo is handled by the caller by
            treating channels as batch, e.g. ``[2, 1, T]``.
        sample_rate : int, optional
            Sample rate in Hz. Defaults to ``self.sample_rate`` when ``None``.

        Returns
        -------
        Tensor[B x C x T']
            Posterior-mean latent with ``C == vae_latent_channels`` (32).
        """
        audio_data = self.preprocess(audio_data, sample_rate)
        z = self.encoder(audio_data)
        if self.attn_proj:
            z = self.pre_block(z.transpose(1, 2)).transpose(1, 2)
        return self.mean_proj(z)
