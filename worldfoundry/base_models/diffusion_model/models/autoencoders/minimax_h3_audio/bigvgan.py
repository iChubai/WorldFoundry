...
"""MiniMax H3 BigVGAN vocoder used after the audio VAE.

SGLang (Apache-2.0) port.  :class:`BigVGAN` reconstructs
32 kHz PCM from DAC latents.  SnakeBeta activations and
:class:`AMPBlock1` are the residual stack.

This is the decoder_type=``bigvgan`` path selected by
:class:`~.minimax_h3.MiniMaxH3AudioVAE`.  Not a video vocoder.

Parallel (but distinct) Snake vocoder: :mod:`~..ltx.audio.vocoder`.
"""
...
def snakebeta(x, alpha, beta):
    """Elementwise SnakeBeta periodic activation."""
    shape = x.shape
...
class SnakeBeta(nn.Module):
    """Learned Snake activation with a beta frequency term."""
    def __init__(
