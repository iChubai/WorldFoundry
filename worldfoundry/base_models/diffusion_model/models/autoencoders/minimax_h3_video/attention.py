...
"""Attention used by the MiniMax H3 ViT video decoder.

SGLang (Apache-2.0) port.  :class:`Attention` is the QKV block
inside :mod:`.base_module` transformer layers.  Rotary
embeddings come from :mod:`.vit_utils`.

Tokens are packed video patches from :mod:`.vae_vit`, not
BCTHW CNN maps.  Encoder attention is CNN-side in
:mod:`.vae_cnn`.

This is H3 video decoder graph math, not a
...
class Attention(nn.Module):
    """Multi-head self-attention for the MiniMax H3 ViT decoder."""
    def __init__(
        self,
        heads,
        dim_head,
