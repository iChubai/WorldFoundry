...
"""ViT decoder transformer blocks for the MiniMax H3 video VAE.

SGLang (Apache-2.0) port.  :class:`TransformerBlock`,
:class:`FeedForward`, and :class:`RotaryEmbeddingND` are the
layers stacked by :class:`~.vae_vit.ViT3DDecoder`.

Tokens are ``N L C`` patch sequences with N-D RoPE.
Causal CNN encode lives in :mod:`.vae_cnn`; this file is
decode-only.

Header-level docs only — residual FFN internals are
checkpoint math.
"""
...
def _scaled_residual_add(residual, x, scale):
    # Single-GPU port: SGLang tried a fused Triton
    # ``try_fused_scaled_residual_add_exact`` kernel and fell back to this exact
    # elementwise form. We keep only the reference form.
    return residual + x * scale


class FeedForward(nn.Module):
    """Transformer FFN (optional gated SiLU) for the H3 ViT decoder."""
    def __init__(
        self,
        dim: int,
    ...
    def _forward_impl(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.use_gated:
            # Single-GPU port: SGLang used a fused
            # ``silu_and_mul_with_activation_rounding`` CUDA kernel on
            # contiguous fp16/bf16 inputs; we keep only the exact eager form.
...
class RotaryEmbeddingND(nn.Module):
    """N-D rotary position embedding table for video patches."""
...
class TransformerBlock(nn.Module):
    """Pre-norm attention + FFN block used by :class:`ViT3DDecoder`."""
    def __init__(
        self,
        heads: int,
        dim_head: int,
