...
"""Torch-native SDPA attention helpers for MiniMax H3 video.

SGLang (Apache-2.0) port.  :func:`flash_attn` calls
``scaled_dot_product_attention``.  :func:`make_block_causal_mask_mod`
builds block-causal masks for the ViT decoder.

Used by :mod:`.attention` / :mod:`.vae_vit`.  There is no
hard dependency on a FlashAttention wheel.

This is H3 video decoder attention runtime, not a conditioner.
"""
import os
...
def _auto_sdpa_backend_name() -> str | None:
    ...
    Ported single-GPU note: SGLang forced SDPBackend.MATH only on ROCm gfx950,
    where fused SDPA corrupts the dense ViT decode. On CUDA (and every other
    platform) SGLang keeps PyTorch's unchanged auto path, i.e. returns None.
    """
    if torch.version.hip is None:
        return None

    # ROCm gfx950 is the only platform SGLang forced onto the MATH backend.
    # WorldFoundry targets single-GPU CUDA; without the sglang gfx95 probe we
    # conservatively keep the default auto path here too.
    return None
...
def make_block_causal_mask_mod(num_tokens, block_size, num_special=0, suffix=False):
    """Build a block-causal attention mask for the H3 ViT decoder."""
    if num_tokens < 0:
...
def flash_attn(
    ...
    """SDPA attention used by MiniMax H3 video (no FlashAttention wheel required)."""
