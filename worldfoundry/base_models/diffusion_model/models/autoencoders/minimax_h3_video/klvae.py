...
"""Ported from SGLang (Apache-2.0) MiniMax H3 video VAE.

MiniMax H3 visual VAE: 3D causal CNN encoder + ViT3D decoder (inference-only
bundle). Ported to single-GPU PyTorch: SGLang distributed tile parallelism
(decode-parallel group / all-gather / ulysses) is removed; the tiled path runs
locally with world_size=1, rank=0, which is what SGLang used by default.
"""
import logging
import math
import os
...
def get_tile_parallel_state():
    # Single-GPU port: SGLang consulted the decode-parallel process group here.
    # WorldFoundry targets a single GPU, so tiling always runs locally.
    return 0, 1
...
class AutoencoderKL(nn.Module):
    ...
    subclass ``AutoencoderKLLegacy``.

    Ported single-GPU port: SGLang derived this from diffusers
    ``ModelMixin, ConfigMixin, FromOriginalModelMixin``. Here it is a plain
    ``nn.Module``; ``self.config`` is a plain dict holding the few derived
    values the original stored via ``@register_to_config``.
    """
    ...
    def tiled_encode(self, x):
        # Single-GPU port: distributed tile parallelism removed; always local.
...
class AutoencoderKLLegacy(AutoencoderKL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Single-GPU port: SGLang called ModelMixin.__init__ to avoid the wrong
        # @register_to_config. Here it is a plain nn.Module; self.config is a
        # dict holding the derived vae ratios.
