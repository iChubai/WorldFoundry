...
"""Torch-native normalization for the MiniMax H3 video VAE.

SGLang (Apache-2.0) port.  :class:`FusedGroupNorm3D`,
:class:`TemporalIsolatedGroupNorm`, and :class:`SpatialNorm3D`
are the 3-D norms used by :mod:`.vae_cnn`.  Factory helpers
select the implementation.

Tensors are BCTHW.  ``fused_*`` names are historical; the
math is plain PyTorch.

This is H3 video encoder graph math, not LTX PixelNorm.
"""
import math
import os
...
def fused_group_norm(x, num_groups, weight, bias, eps=1e-5, activation="silu"):
    """Group-norm helper retained from the SGLang fused-kernel API."""
...
def fused_spatial_norm(
    ...
    """Spatial-norm helper retained from the SGLang fused-kernel API."""
    norm_f = F.group_norm(
        f,
        num_groups,
        weight=norm_weight,
...
class DummyAffine(torch.nn.Module):
    """Identity affine placeholder when a norm has no learned scale/bias."""
...
class TemporalIsolatedGroupNorm(nn.GroupNorm):
    """GroupNorm that does not mix statistics across time."""
    def forward(self, input):
        if input.dim() == 5:
            batch = input.shape[0]
...
class SpatialNorm3D(nn.Module):
    """Spatially conditioned 3-D normalization for the H3 encoder."""
    def __init__(
        self,
        f_channels,
        zq_channels,
...
def get_spatial_norm_3d(
    ...
    *,
    ...
    """Factory for :class:`SpatialNorm3D`."""
...
def get_group_norm_3d(num_channels, use_t_isolated_gn=False):
    """Factory for 3-D group-norm variants used by the H3 encoder."""
    if os.environ.get("MINIMAX_H3_USE_FUSED_NORM", "false").lower() == "true":
        return FusedGroupNorm3D(
            num_groups=32,
            num_channels=num_channels,
