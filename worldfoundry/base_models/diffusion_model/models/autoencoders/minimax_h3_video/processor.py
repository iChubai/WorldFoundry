...
"""Pixel / latent pre- and post-processing for MiniMax H3 video.

:class:`VAEProcessor` plus :func:`get_normalize_transform` /
:func:`get_denormalize_transform` apply the published H3
mean/std to BCTHW RGB or latents.

:func:`get_norm_constants` returns the numeric tables.
Encode/decode networks live in :mod:`.vae_cnn` / :mod:`.vae_vit`.

This is H3 video bookkeeping, sibling of audio latent stats
on :class:`~..minimax_h3_audio.minimax_h3.MiniMaxH3AudioVAE`.
"""
...
def get_norm_constants(
    ...
    """Return published MiniMax H3 video mean/std tables."""
...
def get_normalize_transform(
    ...
    """Build the pixel/latent normalize callable."""
...
def get_denormalize_transform(norm_type: str = "imagenet") -> Normalize:
    """Build the inverse denormalize callable."""
    mean, std = get_norm_constants(norm_type)
...
class VAEProcessor:
    ...
    """Apply H3 mean/std to video pixels or latents."""
    def __init__(
        self,
        *,
        vae_ratio,
