...
"""Causal 3-D CNN encoder for MiniMax H3 video.

SGLang (Apache-2.0) port.  :class:`EncoderFCN3D` is the
public encoder stack; :class:`ResnetBlock3D` /
:class:`Downsample3D` are residual stages.

Pixels in, 24-channel latents out, BCTHW, causal in time.
The decoder is ViT (:mod:`.vae_vit`), not a symmetric CNN.

Header-level docs only; residual stacks are checkpoint math.
"""
...
def norm_silu(x, norm, cond=None):
    """Normalization + SiLU used at residual block boundaries."""
    if cond is None:
...
class Downsample3D(nn.Module):
    """Strided causal 3-D downsample for the H3 video encoder."""
    def __init__(
        self,
        in_channels,
        out_channels,
...
class ResnetBlock3D(nn.Module):
    """3-D residual block used by :class:`EncoderFCN3D`."""
    def __init__(
        self,
        in_channels,
        out_channels=None,
...
class EncoderFCN3D(nn.Module):
    """Causal 3-D CNN encoder that produces MiniMax H3 video latents."""
    def __init__(
        self,
        ch,
        ch_mult,
