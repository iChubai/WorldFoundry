...
"""ViT3D decoder for MiniMax H3 video latents.

SGLang (Apache-2.0) port.  :class:`ViT3DDecoder` reconstructs
BCTHW RGB from 24-channel latents using
:class:`~.base_module.TransformerBlock` and RoPE from
:mod:`.vit_utils`.

Encode is CNN-side (:mod:`.vae_cnn`).  Optional tiling is
configured on :class:`~.model.MiniMaxH3VideoVAE`.

This is the H3 video decode graph, not a text encoder ViT.
"""
...
class ViTBase(nn.Module):
    ...
    ``ModelMixin, ConfigMixin``; here it is a plain ``nn.Module`` and the config
    values that ``forward`` needs are stored as plain attributes.
    """
...
class ViT3DDecoder(ViTBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Single-GPU port: SGLang stored these via diffusers @register_to_config
        # and read them back through self.config.* in forward. We store them as
        ...
        # Single-GPU port: SGLang gated this warning on dist rank 0.
        if len(kwargs) > 0:
