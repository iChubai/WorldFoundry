...
"""Normalization layers used by Sana DC-AE (RMS, BN, LN).

:class:`RMSNorm2d` / :class:`RMSNorm3d`, :class:`LayerNorm2d`,
and :func:`build_norm` are the checkpoint-compatible norms
for 2-D and temporal DC-AE graphs.

Tensors are BCHW (2-D) or BCTHW (3-D).  Optional Triton
kernel lives in :mod:`.triton_rms_norm`.

This is DC-AE graph math, distinct from LTX PixelNorm.
"""
...
from worldfoundry.core.configuration import build_kwargs_from_config
...
class TritonRMSNorm2d(nn.LayerNorm):
    """Checkpoint-compatible RMSNorm2d implemented with native PyTorch.

    The class name is retained because released DC-AE configs use ``trms2d``;
    inference does not need the original custom backward kernel.
    """
    def zero_out(self):
        """Zero out."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            The return value.
        """
        output = x * torch.rsqrt(x.float().square().mean(dim=1, keepdim=True) + self.eps).to(x.dtype)
        if self.elementwise_affine:
            output = output * self.weight.view(1, -1, 1, 1)
