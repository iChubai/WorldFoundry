...
"""2-D residual / MBConv / attention ops for Sana DC-AE.

Large op file: :class:`MBConv`, :class:`FusedMBConv`,
:class:`EfficientViTBlock`, downsample/upsample layers, and
residual wrappers used by :mod:`~..efficientvit.dc_ae`.

Tensors are BCHW image feature maps.  3-D counterparts live
in :mod:`.ops_3d`.  Public classes already have names;
header-level docs only.

Sana DC-AE family only — not Wan residual 3-D blocks.
"""
...
def resize(
    ...
    """Resize a DC-AE feature tensor with mode-compatible arguments."""
    ...
    if mode in {"nearest", "area"}:
        return F.interpolate(value, size=size, scale_factor=scale_factor, mode=mode)
...
class UpSampleLayer(nn.Module):
    """Up sample layer implementation."""
    def __init__(
        self,
        mode="bicubic",
...
class ConvPixelUnshuffleDownSampleLayer(nn.Module):
    """Conv pixel unshuffle down sample layer implementation."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class PixelUnshuffleChannelAveragingDownSampleLayer(nn.Module):
    """Pixel unshuffle channel averaging down sample layer implementation."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class ConvPixelShuffleUpSampleLayer(nn.Module):
    """Conv pixel shuffle up sample layer implementation."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class InterpolateConvUpSampleLayer(nn.Module):
    """Interpolate conv up sample layer implementation."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class ChannelDuplicatingPixelUnshuffleUpSampleLayer(nn.Module):
    """Channel duplicating pixel unshuffle up sample layer implementation."""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class LinearLayer(nn.Module):
    ...
    def _try_squeeze(self, x: torch.Tensor) -> torch.Tensor:
        """Helper function to try squeeze.
...
class ChannelChunkConv3d(nn.Conv3d):
    ...
    def _get_n_chunks(self, numel: int, n_channels: int):
        """Helper function to get n chunks.

        Args:
            numel: The numel.
            n_channels: The n channels.
        """
...
class EfficientViTBlock(nn.Module):
    """Efficient vi t block implementation."""
    def __init__(
        self,
        in_channels: int,
        heads_ratio: float = 1.0,
        ...
        Args:
            ...
            norm: The norm.
            act_func: The act func.
            context_module: The context module.
            local_module: The local module.
            is_video: The is video.
        """
        super().__init__()
