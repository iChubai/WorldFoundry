"""Lightweight Wan 2.2 VAE variant (same 48-ch latent contract).

Pruned / channels-last causal 3-D CNN that still emits
48-channel BCTHW latents with official Wan 2.2 mean/std
(4× time / 8× space).  :class:`Wan2_2_VAE` is the public
surface.

:func:`infer_lightvae_pruning_rate_from_ckpt` reads the
pruning ratio from the checkpoint.  Residual blocks may
differ from :mod:`~..reference_22`.

Use only when the recipe ships a LightVAE weight file.
"""
...
elif hasattr(torch._dynamo.config, "accumulated_recompile_limit"):
...
def _extract_checkpoint_state_dict(raw):
    """Helper function to extract checkpoint state dict.
...
def _map_lightvae_key_to_wanvae(key):
    """Helper function to map lightvae key to wanvae.

    Args:
        key: The key.
    """
    def _map_resnet_tail(tail):
        """Helper function to map resnet tail.
...
def _normalize_vae_state_dict(raw_state):
    """Helper function to normalize vae state dict.

    Args:
        raw_state: The raw state.
    """
...
def infer_lightvae_pruning_rate_from_ckpt(vae_pth, full_decoder_conv1_out=1024):
    ...
    if vae_pth is None or not os.path.exists(vae_pth):
        return None
    try:
        raw_state = load_torch_state_dict(vae_pth, map_location="cpu")
        state = _extract_checkpoint_state_dict(raw_state)
...
def _video_vae(pretrained_path=None, z_dim=16, dim=160, device="cpu", **kwargs):
    ...
    if pretrained_path is not None and os.path.exists(pretrained_path):
        ...
        raw_state = load_torch_state_dict(pretrained_path, map_location="cpu")
...
class Wan2_2_VAE:
    ...
    def __init__(
        ...
        Args:
            ...
            dtype: The dtype.
            device: The device.
            vae_type: The vae type.
            lightvae_pruning_rate: The lightvae pruning rate.
            lightvae_encoder_vae_pth: The lightvae encoder vae pth.
        """
        ...
        if lightvae_encoder_vae_pth is None:
            lightvae_encoder_vae_pth = os.environ.get("WORLDFOUNDRY_WAN22_LIGHTVAE_ENCODER_VAE")
        ...
        if self.vae_type == "wan2.2":
            ...
            if not lightvae_encoder_vae_pth:
                raise RuntimeError(
                    "WORLDFOUNDRY_WAN22_LIGHTVAE_ENCODER_VAE must point to the reviewed Wan2.2 teacher VAE checkpoint."
                )
