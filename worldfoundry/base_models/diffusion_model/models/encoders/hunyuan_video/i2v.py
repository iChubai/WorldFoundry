"""Original HunyuanVideo I2V LLaVA / CLIP text encoder.

Used by :class:`~.component.HunyuanVideoPromptConditioner`
for image-to-video recipes.  :class:`TextEncoder` wraps the
LLaVA-style LLM plus CLIP tokenizer path.

Prompt tokens and optional image features feed the original
Hunyuan DiT.  HunyuanVideo 1.5 should use :mod:`.h15_text`
/ :mod:`.h15_vision` instead.

Checkpoint-compatible wrappers; do not rewrite template
strings that may contain Chinese.
"""
...
def use_default(value, default):
    """Return ``value`` unless it is None, otherwise ``default``."""
...
def load_text_encoder(
    ...
    """Load the original HunyuanVideo I2V LLaVA / CLIP text encoder."""
...
def load_tokenizer(
    ...
    """Load the tokenizer paired with the I2V text encoder."""
...
class TextEncoder(nn.Module):
    """Original HunyuanVideo I2V prompt encoder (LLaVA / CLIP)."""
    def __init__(
        self,
    ...
    def encode(
        ...
        if not self.i2v_mode:
            ...
            input_ids = batch_encoding["input_ids"].to(device)
            # Transformers 4.57+ removed LLaVA's legacy single-placeholder
            # expansion.  The original HunyuanVideo-I2V checkpoint and prompt
            # template intentionally contain one <image> token and expect it
            # to expand to the 24x24 CLIP patch sequence.  Expand explicitly
            # for the newer LLaVA implementation while retaining compatibility
            # with the upstream pinned Transformers 4.48 runtime.
            llava_model = getattr(self.model, "model", None)
            if hasattr(llava_model, "get_placeholder_mask"):
                ...
                for index, row in enumerate(input_ids):
                    positions = torch.where(row == image_token_id)[0]
                    if positions.numel() == image_seq_length:
                        expanded_ids.append(row)
                        if attention_mask is not None:
                            expanded_masks.append(attention_mask[index])
                        continue
                    if positions.numel() != 1:
                        raise ValueError(
                            "HunyuanVideo-I2V expects exactly one <image> "
                            f"placeholder before expansion, found {positions.numel()}"
                        )
                    position = int(positions.item())
                    image_tokens = row.new_full((image_seq_length,), image_token_id)
                    expanded_ids.append(
                        torch.cat((row[:position], image_tokens, row[position + 1 :]))
                    )
                    if attention_mask is not None:
                        image_mask = attention_mask[index].new_full(
                            (image_seq_length,),
                            attention_mask[index, position],
                        )
                        expanded_masks.append(
                            torch.cat(
                                (
                                    attention_mask[index, :position],
                                    image_mask,
                                    attention_mask[index, position + 1 :],
                                )
                            )
                        )
                input_ids = torch.stack(expanded_ids)
                if attention_mask is not None:
                    model_attention_mask = torch.stack(expanded_masks)
            outputs = self.model(
                input_ids=input_ids,
