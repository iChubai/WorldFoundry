...
# Monkey-patch helpers to replace standard linear/feedforward with tiled variants.
...
# Tiled MLP autograd function and helper.
...
class TiledLinear(torch.autograd.Function):
    ...
    def backward(ctx, *grads) -> tuple:
        ...
        x = x.detach()
        # restore requires_grad after detach
        ...
        # flatten to 2D to avoid stride issues when narrowing
...
# Tiled FeedForward nn.Module wrappers.
...
class TiledFeedForward(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        """Forward pass using tiled computation; returns tensor with last dim = dim_out."""
        compute_params = list(self.parameters())
