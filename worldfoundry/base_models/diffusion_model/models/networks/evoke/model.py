...
class EvokeAttnProcessor:
    ...
    def __init__(self):
        ...
        self.kv_arena_enabled = False

    def enable_cache(self, *, use_arena=None):
        self.cache_enabled = True
        if use_arena is not None:
            self.kv_arena_enabled = bool(use_arena)
        self.kv_cache = None
    ...
    def __call__(
        ...
        if attn.restrict_self_attn:
            if use_cache:
                arena = self.kv_cache.get("arena")
                if self._can_use_arena(attn, (key_history, value_history), key, value, enable_navit, sp_plan):
                    if arena is None or arena.layout.current_tokens != key.shape[1]:
                        arena = KVSegmentArena((key_history, value_history), current_tokens=key.shape[1], seq_dim=1)
                        self.kv_cache.update(arena=arena, key_history=arena.static[0], value_history=arena.static[1])
                    arena.validate_persistent((self.kv_cache["key_history"], self.kv_cache["value_history"]))
                    key, value = arena.stage_current((key, value))
                else:
                ...
                if self.cache_enabled and is_first_denoising_step and not enable_navit:
                    ...
                    if key.shape[1] > history_seq_len and self._can_use_arena(
                        attn, (key_history, value_history), key, value, enable_navit, sp_plan
                    ):
                        # History keys have already received their original RoPE.
                        # Own their storage in the arena, rather than retaining
                        # a second history allocation across denoising steps.
                        arena = KVSegmentArena(
                            (key_history, value_history),
                            current_tokens=key.shape[1] - history_seq_len,
                            seq_dim=1,
                        )
                        key, value = arena.stage_current((key[:, history_seq_len:], value[:, history_seq_len:]))
                        self.kv_cache.update(arena=arena, key_history=arena.static[0], value_history=arena.static[1])
        ...
        if _sp_a2a:
            from worldfoundry.base_models.diffusion_model.models.networks.evoke import student_sp as _stu_sp
    ...
    def _can_use_arena(self, attn, history_kv, key, value, enable_navit, sp_plan):
        """Only reuse storage for the ordinary inference cache contract.

        NAViT, sharding, history amplification and autograd retain their
        existing paths. Cache clear/first-step replacement also drop the arena.
        """
        history_key, history_value = history_kv
        return (
            self.kv_arena_enabled
            ...
            and sp_plan is None
            and not attn.is_amplify_history
            and key.shape[1] > 0
            and history_key.dtype == history_value.dtype == key.dtype == value.dtype
            and history_key.device == history_value.device == key.device == value.device
        )
...
class EvokeTransformerBlock(nn.Module):
    def __init__(
        ...
        if cam_ctrl:
            from worldfoundry.base_models.diffusion_model.models.networks.evoke.camera_control import build_camera_modulation_lowrank_submodules
    ...
    def forward(
        ...
        if self.cam_ctrl and cam_token_seq is not None and cam_noise_slots is not None:
            from worldfoundry.base_models.diffusion_model.models.networks.evoke.camera_control import apply_cam_modulation
...
class EvokeTransformer3DModel(
    ...
    def __init__(
        ...
        if (enable_cam_control or self.geo_warp_plucker_enabled) and not hasattr(self, "patch_embedding_wancamctrl"):
            from worldfoundry.base_models.diffusion_model.models.networks.evoke.camera_control import (
    ...
    def enable_kv_cache(self, *, use_arena=None):
        """Enable history caching; optionally set persistent, opt-in arena use."""
        for block in self.blocks:
            if hasattr(block.attn1, "processor") and hasattr(block.attn1.processor, "enable_cache"):
                if use_arena is not None and isinstance(block.attn1.processor, EvokeAttnProcessor):
                    block.attn1.processor.enable_cache(use_arena=use_arena)
                else:
    ...
    def process_input_hidden_states(
        ...
        if getattr(self, "geo_warp_plucker_enabled", False) and cam_plucker_emb is not None:
            if isinstance(latents, list) or isinstance(cam_plucker_emb, list):
                ...
                from worldfoundry.base_models.diffusion_model.models.networks.evoke.camera_control import process_cam_plucker_to_tokens
