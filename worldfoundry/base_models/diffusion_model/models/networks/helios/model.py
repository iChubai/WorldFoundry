...
def _balanced_slice(length: int, rank: int, world_size: int) -> tuple[int, int]:
    """Return the contiguous slice assigned by ``tensor_split`` to one rank."""

    if world_size < 1 or not 0 <= rank < world_size:
        raise ValueError(f"Invalid context-parallel rank {rank} for world size {world_size}.")
    base, remainder = divmod(int(length), int(world_size))
    size = base + int(rank < remainder)
    start = rank * base + min(rank, remainder)
    return start, size


def _shard_history_and_main(
    tensor: torch.Tensor,
    *,
    global_main_length: int,
    rank: int,
    world_size: int,
    dim: int = 1,
) -> tuple[torch.Tensor, int]:
    """Pack one history shard and one main shard for persistent CP execution.

    Helios stores tokens as ``[history | main]``. Splitting that concatenated
    tensor directly gives some ranks history-only shards and makes the block's
    cross-attention boundary incorrect. Split the two semantic regions
    independently, then keep ``[history_rank | main_rank]`` resident through
    the complete transformer stack.
    """

    total_length = int(tensor.shape[dim])
    global_main_length = int(global_main_length)
    history_length = total_length - global_main_length
    if history_length < 0:
        raise ValueError(
            f"Main sequence length {global_main_length} exceeds tensor length {total_length}."
        )
    history_start, history_size = _balanced_slice(history_length, rank, world_size)
    main_start, main_size = _balanced_slice(global_main_length, rank, world_size)
    history = tensor.narrow(dim, history_start, history_size)
    main = tensor.narrow(dim, history_length + main_start, main_size)
    return torch.cat((history, main), dim=dim).contiguous(), main_size


def _context_parallel_runtime(module: nn.Module):
    parallel_config = getattr(module, "_parallel_config", None)
    cp_config = getattr(parallel_config, "context_parallel_config", None)
    mesh = getattr(cp_config, "_flattened_mesh", None)
    if cp_config is None or mesh is None or mesh.size() <= 1:
        return None
    group = mesh.get_group()
    rank = torch.distributed.get_rank(group)
    world_size = torch.distributed.get_world_size(group)
    return group, rank, world_size, bool(cp_config.ulysses_anything)


def _gather_main_tokens(
    tensor: torch.Tensor,
    *,
    group,
    world_size: int,
    ...
    """Gather only projected main tokens, supporting uneven Ulysses shards."""

    tensor = tensor.contiguous()
    if uneven:
        shard_lengths = gather_size_by_comm(int(tensor.shape[dim]), group)
        outputs = []
        for shard_length in shard_lengths:
            shape = list(tensor.shape)
            shape[dim] = int(shard_length)
            outputs.append(torch.empty(shape, device=tensor.device, dtype=tensor.dtype))
    else:
        outputs = [torch.empty_like(tensor) for _ in range(world_size)]
    torch.distributed.all_gather(outputs, tensor, group=group)
...
class HeliosTransformerBlock(nn.Module):
    ...
    def _self_attention_residual(
        ...
        original_context_length, **attention_kwargs,
    ):
    ...
    def forward(
        ...
        **attention_kwargs,
        ...
        hidden_states = self._self_attention_residual(
            hidden_states, norm_hidden_states, gate_msa, rotary_emb,
            original_context_length, **attention_kwargs,
        )
...
class HeliosTransformer3DModel(
    ...
    # Diffusers still injects its parallel attention configuration, while the
    # model forward below owns the semantic history/main sharding. Keeping the
    # generic hook plan empty avoids three full-width split/gather boundaries
    # per block (120 gathers per transformer call).
    _cp_plan = {}
    _block_class = HeliosTransformerBlock
    _history_patch_name = "patch_long"
    _history_patch_scale = 4
    ...
    def __init__(
        ...
        if has_multi_term_memory_patch:
            ...
            setattr(self, self._history_patch_name, nn.Conv3d(
                in_channels,
                self.inner_dim,
                kernel_size=tuple(self._history_patch_scale * p for p in patch_size),
                stride=tuple(self._history_patch_scale * p for p in patch_size),
            ))
    ...
    def _long_history_tokens(self, latents_history_long, indices_latents_history_long, height, width):
        ...
        return latents_history_long, rotary_emb_history_long
    ...
    def forward(
        ...
        if latents_history_long is not None and indices_latents_history_long is not None:
            latents_history_long, rotary_emb_history_long = self._long_history_tokens(
                latents_history_long.to(hidden_states), indices_latents_history_long,
                H1, W1,
            )
        ...
        cp_runtime = _context_parallel_runtime(self)
        if cp_runtime is not None:
            _, rank, world_size, _ = cp_runtime
            local_main_length = None
            sharded_tensors = []
            for tensor in (hidden_states, rotary_emb, temb, timestep_proj):
                sharded, tensor_main_length = _shard_history_and_main(
                    tensor,
                    global_main_length=original_context_length,
                    rank=rank,
                    world_size=world_size,
                    dim=1,
                )
                if local_main_length is None:
                    local_main_length = tensor_main_length
                elif tensor_main_length != local_main_length:
                    raise RuntimeError("Helios context-parallel tensors produced inconsistent main shards.")
                sharded_tensors.append(sharded)
            hidden_states, rotary_emb, temb, timestep_proj = sharded_tensors
            original_context_length = int(local_main_length)
        ...
        for block in self.blocks:
            hidden_states = block(
                ...
                rotary_emb,
                original_context_length,
                **self._block_forward_kwargs(attention_kwargs),
            )
        ...
        if cp_runtime is not None:
            group, _, world_size, uneven = cp_runtime
            hidden_states = _gather_main_tokens(
                hidden_states,
                group=group,
                world_size=world_size,
                uneven=uneven,
