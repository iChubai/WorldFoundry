...
from worldfoundry.core.nn import (
    ConditioningProjection as TextProjection,
)
from worldfoundry.core.nn import (
...
from worldfoundry.core.nn import (
    SinusoidalTimestepEmbedder as TimestepEmbedder,
)
from worldfoundry.core.nn import (
    ...
    def forward(
        ...
        if DISABLE_SP:
            if flash_attn_varlen_func is None:
                raise ImportError("flash-attn is required when Hunyuan GameCraft DISABLE_SP=1.")
    ...
    def __init__(
        ...
        if self.text_projection == "linear":
            self.txt_in = TextProjection(
                self.text_states_dim, self.hidden_size, get_activation_layer("silu"), **factory_kwargs
            )
        elif self.text_projection == "single_refiner":
            self.txt_in = SingleTokenRefiner(self.text_states_dim, hidden_size, num_heads, depth=2, **factory_kwargs)
        else:
        ...
        if camera_condition is not None:
            if latent_len == 18:
                camera_latents = torch.cat(
                    [self.camera_net(torch.zeros_like(camera_condition)), self.camera_net(camera_condition)], dim=1
                )
            elif latent_len == 9:
                ...
                camera_latents = torch.cat(
                    [
                        self.camera_net(torch.zeros_like(camera_condition)[:, 0:4, :, :, :]),
