...
"""Checkpoint-compatible 3D UNet for the released short LVDM trainer.

Latents are ``B C T H W``.  ResBlocks + :class:`SpatialTemporalTransformer`
factorize spatial then temporal attention.  The short trainer forces the
new QKV attention order via :class:`STAttentionBlock`.

Adapted from YingqingHe/LVDM (MIT), commit
 d251dccfbf6352826f5c5681abd86e87ed7e6371, lvdm/models/modules/openaimodel3d.py.
"""
...
from worldfoundry.base_models.diffusion_model.models.networks.lvdm.attention_temporal import (
    SpatialTemporalTransformer,
)
from worldfoundry.base_models.diffusion_model.models.networks.lvdm.attention_temporal import (
    STAttentionBlock2 as _NativeSTAttentionBlock,
)
...
class STAttentionBlock(_NativeSTAttentionBlock):
    """LVDM's released short trainer uses the new QKV attention ordering."""

    def __init__(self, *args, **kwargs):
        kwargs["use_new_attention_order"] = True
        super().__init__(*args, **kwargs)


def nonlinearity(kind: str = "silu") -> nn.Module:
    if kind == "silu":
        return nn.SiLU()
    if kind == "leaky_relu":
        return nn.LeakyReLU()
    raise ValueError(f"unsupported LVDM nonlinearity: {kind!r}")
...
def make_spatialtemporal_transformer(module_name="attention_temporal", class_name="SpatialTemporalTransformer"):
    if module_name != "attention_temporal" or class_name != "SpatialTemporalTransformer":
        raise ValueError("the native LVDM short UNet supports SpatialTemporalTransformer from attention_temporal")
    global STTransformerClass
    STTransformerClass = SpatialTemporalTransformer
    return STTransformerClass


# ---------------------------------------------------------------------------------------------------
class LVDMShortUNet(nn.Module):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_temporal_transformer:
            assert context_dim is not None, "context_dim is required with the temporal transformer"
        if context_dim is not None:
            assert use_temporal_transformer, "context_dim requires the temporal transformer"
