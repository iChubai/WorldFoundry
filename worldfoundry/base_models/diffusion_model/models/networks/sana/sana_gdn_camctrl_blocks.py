...
Dual-branch video attention for Sana-WM: a main GDN scan plus a camera
branch whose Q/K/V are transformed by UCPE ray matrices.  Softmax ablations
(``*SoftmaxUCPE*``) swap the GDN recurrence for SDPA.
...
def _camera_execution_weight_bias(module: nn.Module) -> tuple[torch.Tensor, torch.Tensor | None]:
    """Return active parameters for plain modules and VRAM-managed wrappers."""
...
def _sdpa_unmasked_with_pad(
    ...
    """Run ``_worldfoundry_scaled_dot_product_attention(q, k, v)`` with FA-friendly head_dim padding.
    ...
    out = _worldfoundry_scaled_dot_product_attention(q, k, v)
    if _need_pad:
        out = out[..., :D]
    return out
...
def torch_chunk_cam_single_path_delta_rule(
    ...
    def _chunk_scan_kv(w_kv: torch.Tensor, u_kv: torch.Tensor, s_kv: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Helper function to chunk scan kv.
    ...
    **Shared with main branch** (no duplication):
    ...
    def init_cam_branch_weights(self) -> None:
        ...
        if self.cam_dim != self.dim * self.heads:
            logging.getLogger(__name__).warning(
                "Skipping camera-branch initialization: cam_dim (%s) != dim (%s) * heads (%s)",
                self.cam_dim,
                self.dim,
                self.heads,
            )
        ...
        logging.getLogger(__name__).debug(
            "Initializing camera-branch QKV for %s", self.__class__.__name__
        )
        w = self.qkv.weight
        b = self.qkv.bias
        dim = self.cam_dim
    ...
    def _prepare_cam_qkv(
        ...
        **kwargs: object,
        ...
        q_w, q_b = _camera_execution_weight_bias(self.q_proj_cam)
        k_w, k_b = _camera_execution_weight_bias(self.k_proj_cam)
        v_w, v_b = _camera_execution_weight_bias(self.v_proj_cam)
...
class BidirectionalGDNUCPELiteLAPostUCPERenorm(BidirectionalGDNUCPELiteLA):
    ...
    def _stabilize_cam_transforms(
        ...
        """Helper function to stabilize cam transforms.

        Args:
            q_cam: The q cam.
            k_cam: The k cam.
            v_cam: The v cam.
            q_cam_trans: The q cam trans.
            k_cam_trans: The k cam trans.
            v_cam_trans: The v cam trans.
...
def _forward_cam_branch_softmax(
    ...
    **kwargs,
    ...
    Uses ``_worldfoundry_scaled_dot_product_attention`` with optional invalid-key masking.
    """
    B, N, _ = x.shape
    T, H, W = HW
    S = H * W
    ...
    out = _worldfoundry_scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, attn_mask=invalid_kv_logit_bias)
    if _need_pad:
        out = out[..., :D]
...
class _SoftmaxUCPESinglePathLiteLA(
    ...
    Replaces GDN recurrence with ``_worldfoundry_scaled_dot_product_attention``.
