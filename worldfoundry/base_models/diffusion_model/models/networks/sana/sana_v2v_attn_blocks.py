...
Hybrid of bidirectional GDN (``V2VBiGDNAttention``, optional state cache)
and gated softmax (``V2VGatedSoftmaxAttention`` / after-RoPE variant).
``SanaMSVideoV2V`` picks a subset of layers for softmax; the rest stay GDN.
...
def _bidirectional_triton_gdn_is_supported(version: str | None = None) -> bool:
    """Return whether Triton supports the bidirectional fused dot layouts."""
...
def _get_cache_frame_index(cache_frame_indices, *, T: int) -> int | None:
    ...
    assert int(cache_frame_indices.numel()) == 1, (
...
class V2VBiGDNAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: object,
        ...
        if self.gdn_variant != "pytorch" and not _bidirectional_triton_gdn_is_supported():
            # Triton 3.1 aborts inside LLVM for the bidirectional dot layout,
            # so this must be selected before compilation rather than caught.
            self.gdn_variant = "pytorch"
...
class V2VAfterRoPEGatedSoftmaxAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: object,
        ...
        q_weight, q_bias = _execution_weight_bias(self.q)
        padding_size = self.q.kernel_size[0] // 2
        center_q_weight = q_weight[:, :, padding_size : padding_size + 1]
        q = F.conv1d(
            x_qk,
            center_q_weight,
            bias=q_bias,
        ...
        k_weight, k_bias = _execution_weight_bias(self.k)
        padding_size = self.k.kernel_size[0] // 2
        center_k_weight = k_weight[:, :, padding_size : padding_size + 1]
        k = F.conv1d(
            x_qk,
            center_k_weight,
            bias=k_bias,
