...
"""Wan 2.1 action-conditioned DiT (official list-of-videos packing).

Extends the Wan 2.1 stem with :class:`ActionModule` mouse/keyboard tokens
injected inside each attention block.  Self-attention still uses 3D RoPE
and flash attention from :mod:`.action_attention_21`.

Adds action conditioning.  No VACE, camera ControlNet, causal KV cache,
or linear-attention processors live here.
"""
import math
import numpy as np
import torch
...
class WanModel(nn.Module, NativeConfigMixin):
    r"""Wan 2.1 T2V/I2V DiT with per-block :class:`ActionModule` residuals."""
