...
"""AnyFlow / FlowMap / FAR Wan graph (Diffusers-shaped, native training).

NVIDIA AnyFlow Wan transformer with FlexAttention block masks,
dual timestep embeddings, and cache mixins.  Used by WorldFoundry
FlowMap training.  Adds flow-map attention masks; not VACE.
"""
# Copied from NVlabs/AnyFlow commit d2acf7373a45173082ec47eb16553a373b10f856;
# the author implementation was modified from diffusers v0.35.1 Wan.
...
def compiled_flex_attention():
    """Compile the FAR attention kernel only when a causal call needs it."""
...
class WanSelfAttnProcessor2_0:
    """AnyFlow self-attention: SDPA or FlexAttention, plus optional FAR KV cache."""

    def __init__(self):
        if not hasattr(F, 'scaled_dot_product_attention'):
            raise ImportError('WanSelfAttnProcessor2_0 requires PyTorch 2.0. To use it, please upgrade PyTorch to 2.0.')
    ...
    def __call__(
        ...
        if attention_mask is None:
            ...
            hidden_states = compiled_flex_attention()(query, key, value, block_mask=attention_mask)[:, :, :seq_len]
...
class WanCrossAttnProcessor2_0:
    """AnyFlow text/image cross-attention processor (PyTorch 2 SDPA)."""

    def __init__(self):
        if not hasattr(F, 'scaled_dot_product_attention'):
            raise ImportError('WanCrossAttnProcessor2_0 requires PyTorch 2.0. To use it, please upgrade PyTorch to 2.0.')
...
class WanImageEmbedding(torch.nn.Module):
    """Project CLIP / image tokens into AnyFlow hidden size."""
...
class WanTimeTextImageEmbedding(nn.Module):
    """Per-frame timestep + text (+ image) embeddings for FAR / AnyFlow."""
    ...
    def forward(
        ...
        if is_causal:
            ...
            if clean_timestep is not None:
                clean_timestep, clean_timestep_proj = self.forward_timestep(clean_timestep, encoder_hidden_states, far_cfg['full_token_per_frame'])  # noqa: E501
...
class WanTwoTimeTextImageEmbedding(nn.Module):
    """Gated mix of ``t`` and delta-time embeddings (AnyFlow ``r`` / ``t-r``)."""
...
class WanTransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            out_bias=True,
            processor=WanSelfAttnProcessor2_0(),
        )
        ...
        self.attn2 = Attention(
            ...
            processor=WanCrossAttnProcessor2_0(),
        )
...
class AnyFlowWanTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    r"""
    ...
    def __init__(
        ...
        compressed_patch_size: Optional[Tuple[int]] = None,
        full_chunk_limit: Optional[int] = None,
        ...
        self.rope = WanRotaryPosEmbed(attention_head_dim, patch_size, compressed_patch_size, rope_max_seq_len)
        ...
        if self.config.compressed_patch_size is not None:
            self.setup_far_model()
        if self.config.deltatime_type is not None:
            self.setup_flowmap_model()

    def setup_flowmap_model(self):
    ...
    def setup_far_model(self):
        ...
        # reset rope part with compressed patch size
        self.rope = WanRotaryPosEmbed(self.config.attention_head_dim, self.config.patch_size, self.config.compressed_patch_size, self.config.rope_max_seq_len)
    ...
    def forward(self, *args, **kwargs):
        ...
        if is_causal:
            ...
            return self._forward_bidirection(*args, is_causal=False, **kwargs)
...
__all__ = ["AnyFlowWanTransformer3DModel"]
