...
"""Flash + block-sparse attention kernels for linear-attention Wan.

Used by :mod:`.linear`.  Kernel adapter only (flash, block-sparse, SDPA).
"""
import torch
...
def block_sparse_attention(
    ...
    if block_mask is not None:
        if block_mask.ndim == 2:
            block_mask = block_mask[None, None].repeat(b, num_heads, 1, 1)
        elif block_mask.ndim == 3:
            block_mask = block_mask.repeat(b, 1, 1)
        else:
            raise ValueError(f"Block mask must be 2D or 3D, but got {block_mask.ndim}D")
