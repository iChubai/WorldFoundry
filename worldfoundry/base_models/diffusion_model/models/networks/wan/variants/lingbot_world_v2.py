"""LingBot-World-V2 causal-fast DiT (packed / official hybrid).

V2-specific causal backbone; common Wan layers come from WorldFoundry.
Adds causal / fast-attention hooks.  Licensed CC BY-NC-SA 4.0 upstream.
"""
import math
...
__all__ = ["LingBotWorldV2Model", "WanModelFast"]
...
class WanCrossAttention(WanSelfAttention):
    def forward(self, x, context, context_lens, crossattn_cache=None, cross_attn_first_call=None):
        r"""
        Args:
...
class CausalWanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        def cross_attn_ffn(x, context, context_lens, e, crossattn_cache=None, cross_attn_first_call=None):
            x = x + self.cross_attn(
                self.norm3(x),
                context,
                context_lens,
        ...
        x = cross_attn_ffn(x, context, context_lens, e, crossattn_cache, cross_attn_first_call=cross_attn_first_call)
        return x
...
class WanModelFast(nn.Module, NativeConfigMixin):
    r"""LingBot-World-V2 causal-fast DiT."""
...
LingBotWorldV2Model = WanModelFast
