...
"""Native flow-matching UniPC solver shared by diffusion model families.

Training-free multistep solver shared by Wan, Cosmos3, and MAGI-2 adapters.
The runner sees only ``SchedulerStep``; solver history lives on this object.
"""
...
def _solve_small_system_on_cpu(
    matrix: torch.Tensor,
    rhs: torch.Tensor,
    *,
    device: torch.device,
    ...
    """Solve UniPC's tiny coefficient system without creating a cuSOLVER handle.

    UniPC solves at most a handful of scalar coefficients per step.  Running
    that solve on CUDA needlessly initializes cuSOLVER at the point where the
    denoiser is already close to peak memory, which can fail even though the
    actual system is only 2x2 or 3x3.  The schedule tensors are CPU float32
    values already, so keep the solve there and transfer only its coefficients.
    """

    solution = torch.linalg.solve(matrix.to(device="cpu"), rhs.to(device="cpu"))
    return solution.to(device=device, dtype=dtype)


class FlowUniPCMultistepScheduler(NativeSchedulerMixin):
    """
    `UniPCMultistepScheduler` is a training-free framework designed for the fast sampling of diffusion models.

    The scheduler owns a lightweight native inference configuration; it does
    not depend on Diffusers lifecycle, serialization, or Hub mixins.
    ...
    Args:
        ...
        solver_p (`NativeSchedulerMixin`, default `None`):
    ...
    def __init__(
        ...
        solver_p: NativeSchedulerMixin = None,
        ...
        Args:
            ...
            solver_order: The solver order.
            prediction_type: The prediction type.
            shift: The shift.
            use_dynamic_shifting: The use dynamic shifting.
            thresholding: The thresholding.
            dynamic_thresholding_ratio: The dynamic thresholding ratio.
            sample_max_value: The sample max value.
            predict_x0: The predict x0.
            solver_type: The solver type.
            lower_order_final: The lower order final.
            disable_corrector: The disable corrector.
            solver_p: The solver p.
            timestep_spacing: The timestep spacing.
            steps_offset: The steps offset.
            final_sigmas_type: The final sigmas type.
        """
    ...
    def _sigma_to_alpha_sigma_t(self, sigma):
        """Helper function to sigma to alpha sigma t.
    ...
    def multistep_uni_p_bh_update(
        ...
        **kwargs,
        ...
        rks = torch.stack(rks).to(device="cpu")
        ...
        if len(D1s) > 0:
            ...
            if order == 2:
                ...
                rhos_p = _solve_small_system_on_cpu(
                    R[:-1, :-1],
                    b[:-1],
                    device=device,
                    dtype=x.dtype,
    ...
    def multistep_uni_c_bh_update(
        ...
        **kwargs,
        ...
        if order == 1:
            ...
            rhos_c = _solve_small_system_on_cpu(
                R,
                b,
                device=device,
                dtype=x.dtype,
    ...
    def index_for_timestep(self, timestep, schedule_timesteps=None):
        ...
        Args:
            ...
            schedule_timesteps: The schedule timesteps.
        """
    ...
    def add_noise(
        ...
        if self.begin_index is None:
            ...
            step_indices = [self.index_for_timestep(t, schedule_timesteps) for t in timestep_values]
