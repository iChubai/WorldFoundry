...
# (FlowUniPCMultistepScheduler) + inference/pipeline/inference_engine.py
# (ZeroSNRDDPMDiscretization). Distributed coupling stripped; math preserved.

"""Flow-matching UniPC multistep scheduler for MAGI-2-preview (single-GPU).

Faithful port of the flow-matching UniPC solver used by the MAGI-2-preview
audio-video sampler, plus the ZeroSNR-DDPM discretization used only to pick the
refiner renoise sigma (``magi2_refiner_noise_value``).
"""
...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_ddim(
        ...
        """
        Propagate the sample with a DDIM-style flow update from the learned velocity.
        """
    ...
    def step_sde(
        ...
        """
        Propagate the flow process from the learned velocity with an SDE update
        ...
        Args:
            ...
            t (`int`):
                The current discrete timestep index in the diffusion chain.
...
# -----------------------------------------------------------------------------
# Refiner noise schedule (ZeroSNR-DDPM) — used only to pick the refiner renoise
# sigma via ``magi2_refiner_noise_value`` (default 220).
# -----------------------------------------------------------------------------


class ZeroSNRDDPMDiscretization:
    """Zero-terminal-SNR DDPM discretization.

    In MAGI-2-preview this is instantiated once and called as
    ``ZeroSNRDDPMDiscretization()(1000, do_append_zero=False, flip=True)`` to
    build the 1000-entry refiner sigma table; the refiner then renoises the
    upscaled latent with ``sigma = sigmas[noise_value]`` (``noise_value=220``).
    """
...
# Namespaced aliases so the schedulers package lazy table can register these
# without colliding with the existing shared ``FlowUniPCMultistepScheduler``
# (``.flow_unipc``) / other scheduler names.
Magi2FlowUniPCMultistepScheduler = FlowUniPCMultistepScheduler
Magi2ZeroSNRDDPMDiscretization = ZeroSNRDDPMDiscretization


__all__ = [
    "FlowUniPCMultistepScheduler",
    "Magi2FlowUniPCMultistepScheduler",
    "Magi2ZeroSNRDDPMDiscretization",
