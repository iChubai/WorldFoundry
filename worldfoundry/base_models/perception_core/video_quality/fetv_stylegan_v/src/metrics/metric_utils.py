...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None,
                       progress=None, cache=True, gen_dataset_kwargs={}, generator_as_dataset=False, seed=0, real_feat_path=None, fake_feat_path=None):
        ...
        self.seed                     = seed
        self.real_feat_path           = real_feat_path
        self.fake_feat_path           = fake_feat_path
...
def compute_feature_stats_for_dataset(
    ...
    feature_stats_cls=FeatureStats, feat_path=None, **stats_kwargs):
    ...
    if feat_path is None:
        ...
        features = torch.load(feat_path).to(opts.device)
        num_items = len(features)
    if max_items is not None:
        random.seed(opts.seed)
        sample_items = random.sample(list(range(num_items)), max_items) if max_items<num_items else None
    ...
    if sample_items is not None:
        item_subset = [sample_items[i] for i in item_subset]
    if feat_path is not None:
        features = features[item_subset]
