...
def default_bpe():
    ...
    candidates = [
        os.environ.get("WORLDFOUNDRY_VICLIP_BPE_PATH"),
        str(Path(tokens["WORLDFOUNDRY_CKPT_DIR"]) / "VBench" / "ViCLIP" / "bpe_simple_vocab_16e6.txt.gz"),
        str(Path(tokens["WORLDFOUNDRY_CKPT_DIR"]) / "ViCLIP" / "bpe_simple_vocab_16e6.txt.gz"),
        str(Path(tokens["WORLDFOUNDRY_CACHE_DIR"]) / "models" / "vbench" / "ViCLIP" / "bpe_simple_vocab_16e6.txt.gz"),
    ]
    ...
    raise FileNotFoundError(
        "ViCLIP tokenizer BPE file is not staged: bpe_simple_vocab_16e6.txt.gz\n"
        f"Searched:\n{searched}\n"
        "Set WORLDFOUNDRY_VICLIP_BPE_PATH to the file path if it is stored elsewhere."
    )
