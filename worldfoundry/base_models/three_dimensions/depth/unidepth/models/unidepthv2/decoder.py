...
from ...layers import MLP, AttentionBlock, ConvUpsampleShuffleResidual, NystromBlock, PositionEmbeddingSine
from ...utils.geometric import flat_interpolate, generate_rays
from ...utils.positional_embedding import generate_fourier_features
    ...
    def __init__(self, input_dims: list[int], hidden_dim: int):
        ...
        for input_dim in input_dims:
            self.input_adapters.append(
                nn.Sequential(nn.LayerNorm(input_dim), nn.Linear(input_dim, hidden_dim), nn.GELU())
            )
...
class CameraHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        self.aggregate1 = AttentionBlock(hidden_dim, num_heads=1, expansion=expansion, dropout=dropout)
        self.aggregate2 = AttentionBlock(hidden_dim, num_heads=1, expansion=expansion, dropout=dropout)
        self.latents_pos = nn.Parameter(torch.randn(1, 4, hidden_dim), requires_grad=True)
    ...
    def fill_intrinsics(self, x):
        camera_intrinsics = torch.zeros(x.shape[0], 3, 3, device=x.device, requires_grad=False)
    ...
    def forward(self, features, cls_tokens, pos_embed) -> torch.Tensor:
        ...
        cls_tokens = self.aggregate1(cls_tokens, context=features, pos_embed=latents_pos)
        cls_tokens = self.aggregate2(cls_tokens, context=features, pos_embed=latents_pos)
...
class DepthHead(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.to_depth = nn.Conv2d(16 * len(depths), 1, 7, padding=3, padding_mode="reflect")
        self.to_confidence = nn.Conv2d(16 * len(depths), 1, 7, padding=3, padding_mode="reflect")
    ...
    def project_rays(self, rays, shapes):
        ...
        for i, layer in enumerate(self.rays_layers):
            embedded_rays.append(layer(self.embed_rays(rays, [(2**i) * x for x in shapes])))
        return embedded_rays

    def decode_depth(self, latents_16, rays, shapes):
        latents = latents_16
        out_features, depths, confidences = [], [], []
        for i, (up, layers, rays_embedding) in enumerate(zip(self.ups, self.process_layers, rays)):
            for layer in layers:
        ...
        for i, (layer, features) in enumerate(zip(self.depth_mlp[::-1], out_features[::-1])):
            out_depth_features = layer(features).permute(0, 3, 1, 2)
            out_depth_features = F.interpolate(out_depth_features, size=self.original_shapes, mode="bilinear")
            depths.append(out_depth_features)
        logdepth = self.to_depth(torch.cat(depths, dim=1))

        # aggregate output and project to confidences
        for i, (layer, features) in enumerate(zip(self.confidence_mlp[::-1], out_features[::-1])):
            out_conf_features = layer(features).permute(0, 3, 1, 2)
            out_conf_features = F.interpolate(out_conf_features, size=self.original_shapes, mode="bilinear")
    ...
    def forward(self, features: torch.Tensor, rays_hr: torch.Tensor, pos_embed, level_embed) -> torch.Tensor:
        features = features.unbind(dim=-1)
        shapes = self.shapes
...
class Decoder(nn.Module):
    ...
    def get_adapted_features(self, features_flat, splits):
        ...
        features_projected = self.input_adapter(features_flat_cat, splits)  # list [b hw c] shapes
    ...
    def run_camera(self, cls_tokens, features, pos_embed, original_shapes, rays_gt):
        ...
        cls_tokens = torch.cat(torch.chunk(cls_tokens, cls_tokens_splits.shape[0], dim=-1), dim=1)
        ...
        rays = rays_gt if rays_gt is not None else generate_rays(intrinsics, original_shapes)[0]
    ...
    def run_global(self, cls_tokens, features, rays):
        ...
        scale, shift = self.global_layer(features=features, rays=rays, cls_tokens=cls_tokens)
    ...
    def forward(self, inputs, image_metas) -> torch.Tensor:
        ...
        level_shapes = sorted(list(set([tuple([x.shape[1], x.shape[2]]) for x in inputs["features"]])))[::-1]
        if len(level_shapes) == 1:
            level_shapes = level_shapes * self.num_resolutions
        input_shapes = [
            level_shapes[i] for i, (start, end) in enumerate(self.slices_encoder) for _ in range(end - start)
        ]
        common_shape = level_shapes[-2]

        # input shapes repeat shapes for each level, times the amount of the layers:
        features_flat = [
            flat_interpolate(rearrange(x, "b h w c -> b (h w) c"), old=input_shape, new=common_shape)
        ...
        scale, shift = self.run_global(inputs["global_tokens"], features=features, rays=rays)
        ...
        depth = (depth_normalized + shift) * scale  # shift is scale invariant if we do (x + mu) * sigma
    ...
    def build(self, config):
        ...
        self.level_embeds = nn.Parameter(torch.randn(len(input_dims), hidden_dim), requires_grad=True)
