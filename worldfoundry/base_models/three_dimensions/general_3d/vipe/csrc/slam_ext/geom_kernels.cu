...
#define GPU_1D_KERNEL_LOOP(k, n) for (size_t k = threadIdx.x; k < n; k += blockDim.x)
...
__device__ void adjSE3(const float *t, const float *q, const float *X, float *Y) {
...
__device__ void relSE3(const float *ti, const float *qi, const float *tj, const float *qj, float *tij, float *qij) {
...
__device__ void expSO3(const float *phi, float *q) {
    // SO3 exponential map
...
__device__ void crossInplace(const float *a, float *b) {
    float x[3] = {
        a[1] * b[2] - a[2] * b[1],
        a[2] * b[0] - a[0] * b[2],
        a[0] * b[1] - a[1] * b[0],
...
__device__ void expSE3(const float *xi, float *t, float *q) {
    // SE3 exponential map
...
__global__ void projective_transform_kernel(
    ...
    GPU_1D_KERNEL_LOOP(k, ht * wd) {
        ...
        // Apply Eq.(12) to get dl_dTi
        adjSE3(tij, qij, Jj, Ji);
        for (int n = 0; n < 6; n++) Ji[n] *= -1;

        // Compute the 'ij' half of the JTJ.
        l = 0;
        for (int n = 0; n < 12; n++) {
            for (int m = 0; m <= n; m++) {
                hij[l] += wu * Jx[n] * Jx[m];
        ...
        // Compute RHS of pose, and two parts of the LHS of pose-depth.
        for (int n = 0; n < 6; n++) {
            vi[n] += wu * ru * Ji[n];
            vj[n] += wu * ru * Jj[n];
            if (optimize_intrinsics) {
...
__global__ void frame_distance_kernel(
    ...
    __shared__ float fxi;
    __shared__ float fyi;
    __shared__ float cxi;
    __shared__ float cyi;
    __shared__ float fxj;
    __shared__ float fyj;
    __shared__ float cxj;
    __shared__ float cyj;
    ...
    // load intrinsics from global memory
    if (thread_id == 0) {
        ix = static_cast<int>(pi[block_id]);
        jx = static_cast<int>(pj[block_id]);
        dix = static_cast<int>(di[block_id]);
        rix = static_cast<int>(ri[block_id]);
        rjx = static_cast<int>(rj[block_id]);
        fxi = intrinsics[rix][0];
        fyi = intrinsics[rix][1];
        cxi = intrinsics[rix][2];
        cyi = intrinsics[rix][3];
        fxj = intrinsics[rjx][0];
        fyj = intrinsics[rjx][1];
        cxj = intrinsics[rjx][2];
        cyj = intrinsics[rjx][3];
    }
    ...
    accum[thread_id] = 0;
    __shared__ float valid[THREADS];
    valid[thread_id] = 0;
    __shared__ float total[THREADS];
    total[thread_id] = 0;
    ...
    for (int n = 0; n < 1; n++) {
        ...
        GPU_1D_KERNEL_LOOP(k, ht * wd) {
            ...
            du = fxj * (Xj[0] / Xj[2]) + cxj - u;
            dv = fyj * (Xj[1] / Xj[2]) + cyj - v;
            d = sqrtf(du * du + dv * dv);
...
__global__ void depth_filter_kernel(const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> poses,
    ...
    if (index < ht * wd) {
        ...
        if (u0 >= 0 && v0 >= 0 && u0 < wd - 1 && v0 < ht - 1) {
            ...
            if (abs(1.0 / dj - 1.0 / d00) < t)
                atomicAdd(&counter[block_id][i][j], 1.0f);
            else if (abs(1.0 / dj - 1.0 / d01) < t)
                atomicAdd(&counter[block_id][i][j], 1.0f);
            else if (abs(1.0 / dj - 1.0 / d10) < t)
                atomicAdd(&counter[block_id][i][j], 1.0f);
            else if (abs(1.0 / dj - 1.0 / d11) < t)
                atomicAdd(&counter[block_id][i][j], 1.0f);
        }
    }
}

__global__ void iproj_kernel(const torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> poses,
                             const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> disps,
                             const torch::PackedTensorAccessor32<float, 1, torch::RestrictPtrTraits> intrinsics,
...
__device__ void retrSE3(const float *xi, const float *t, const float *q, float *t1, float *q1) {
...
__global__ void pose_retr_kernel(torch::PackedTensorAccessor32<float, 2, torch::RestrictPtrTraits> poses,
...
torch::Tensor accum_cuda(torch::Tensor data, torch::Tensor ix, torch::Tensor jx) {
    ...
    for (int j = 0; j < count; j++) {
        while (i < ix.size(0) && ix_data[kx_data[i]] <= jx_data[j]) {
            if (ix_data[kx_data[i]] == jx_data[j]) cols.push_back(kx_data[i]);
            i++;
        }
...
__global__ void EvT6x1_kernel(const torch::PackedTensorAccessor32<float, 3, torch::RestrictPtrTraits> E,
    ...
    if (idx[blockIdx.x] <= 0 || idx[blockIdx.x] >= x.size(0)) return;
...
class SparseBlock {
   public:
    ...
    SparseBlock operator-(const SparseBlock &S) { return SparseBlock(A - S.A, b - S.b, N, M); }

    std::tuple<torch::Tensor, torch::Tensor> get_dense() {
        Eigen::MatrixXd Ad = Eigen::MatrixXd(A);

        torch::Tensor H = torch::from_blob(Ad.data(), {N * M, N * M}, torch::TensorOptions().dtype(torch::kFloat64))
                              .to(torch::kCUDA)
                              .to(torch::kFloat32);

        torch::Tensor v = torch::from_blob(b.data(), {N * M, 1}, torch::TensorOptions().dtype(torch::kFloat64))
                              .to(torch::kCUDA)
                              .to(torch::kFloat32);
    ...
    torch::Tensor solve(const float lm = 0.0001, const float ep = 0.1) {
        ...
        if (solver.info() == Eigen::Success) {
            ...
            dx = torch::from_blob(x.data(), {N, M}, torch::TensorOptions().dtype(torch::kFloat64))
                     .to(torch::kCUDA)
                     .to(torch::kFloat32);
        } else {
            dx = torch::zeros({N, M}, torch::TensorOptions().device(torch::kCUDA).dtype(torch::kFloat32));
        }
...
std::vector<torch::Tensor> ba_cuda_impl(torch::Tensor poses, torch::Tensor disps, torch::Tensor intrinsics,
    ...
    // jtj of pose (LHS and RHS)
    torch::Tensor Hs = torch::zeros({4, num, 6, 6}, opts);  // Real JTJ = Hs + Hs.T.
    torch::Tensor vs = torch::zeros({2, num, 6}, opts);     // This is the real vs.
    // Cross-jtj-term of pose and depth. Actual JTJ_ii = sum_j E_ij + E_ii, JTJ_ij = E_ij.
...
torch::Tensor iproj_cuda(torch::Tensor poses, torch::Tensor disps, torch::Tensor intrinsics) {
    ...
    iproj_kernel<<<blocks, THREADS>>>(poses.packed_accessor32<float, 2, torch::RestrictPtrTraits>(),
                                      disps.packed_accessor32<float, 3, torch::RestrictPtrTraits>(),
                                      intrinsics.packed_accessor32<float, 1, torch::RestrictPtrTraits>(),
