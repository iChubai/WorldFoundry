...
class Pi3X(nn.Module, PyTorchModelHubMixin):
    def __init__(self, ckpt=None, use_multimodal=True):
        super().__init__()
    ...
    def encode(
        ...
        hidden = self.encoder.forward_features(imgs)["x_norm_patchtokens"]
        ...
        if self.use_multimodal:
            with torch.amp.autocast(device_type="cuda", enabled=False):
                ...
                if rays is not None:
                    ...
                    if intrinsics is None:
                        ...
                        pix = (
                            torch.from_numpy(get_pixel(H, W).T.reshape(H, W, 3))
                            .to(device)
                            .float()[None]
                            .repeat(B, 1, 1, 1)
                        )
            ...
            if mask_add_depth.sum() > 0:
                depth_emb = (
                    self.depth_encoder.forward_features(torch.cat([normalized_depths, depths_masks], dim=1))[
                        "x_norm_patchtokens"
                    ]
                    + self.depth_emb
                )
    ...
    def forward_head(self, hidden, pos, B, N, H, W, patch_h, patch_w):
        ...
        ret_metric = self.metric_decoder(
            self.metric_token.repeat(B, 1, 1), hidden.reshape(B, N * hw, -1), xpos=pos_hw[:, 0:1], ypos=pos_hw
        )
        ...
        with torch.amp.autocast(device_type="cuda", enabled=False):
            ...
            camera_poses = self.camera_head(ret_camera[:, self.patch_start_idx :].float(), patch_h, patch_w).reshape(
                B, N, 4, 4
            )
            ...
            # points
            points = torch.einsum("bnij, bnhwj -> bnhwi", camera_poses, homogenize_points(local_points))[
                ..., :3
            ] * metric.view(B, 1, 1, 1, 1)
            ...
            if self.use_multimodal:
                if i in [1, 9, 17, 25, 33] and use_pose_mask.sum() > 0:
                    ...
                    poses_feat = self.pose_inject_blk[pose_inject_blk_idx](
                        hidden[..., self.patch_start_idx :, :].reshape(B, N * (hw - self.patch_start_idx), -1),
                        poses,
                        H,
                        W,
                        H // 14,
