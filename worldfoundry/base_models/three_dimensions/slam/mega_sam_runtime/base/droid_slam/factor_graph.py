...
class FactorGraph:
  ...
  def update(
    ...
    self.net, delta, weight_, damping, upmask, mot_prob, refined_w = (
        self.update_op(
            self.net,
            self.inp,
            corr,
    ...
    with torch.cuda.amp.autocast(enabled=False):
      ...
      damping = (
          DAMPING_FACTOR * self.damping[torch.unique(ii)].contiguous() + EP
      )
  ...
  def update_lowmem(
    ...
    for step in range(steps):
      ...
      #         motion_only=False,
      #         opt_intr=opt_intr,
      #         alpha=alpha)#True if step > 15 else False)
      # else:
      # print("full_frame_ba ", full_frame_ba)
      # if ba_sgd: #and step < 10:
      #     error = self.video.ba(
      #             target, weight,
      #             damping + 1e5,
      #             self.ii, self.jj, 1, t,
