...
features ``[d × o, d]`` from ``[fx, fy, cx, cy]`` and camera-to-world poses.
...
def custom_meshgrid(*args):
    """``ij``-indexed meshgrid; omit ``indexing=`` on torch < 1.10."""
...
def ray_condition(
    ...
    """Build Plücker rays ``[d × o, d]`` with shape ``[B, V, H, W, 6]``.

    ``K`` is ``[B, V, 4]`` as ``[fx, fy, cx, cy]``. Pixel centers are offset by
    ``+0.5``. When ``flip_flag`` is set, those views sample U from ``W-1`` down
    to ``0`` so a horizontal flip does not invert the camera rays.
    """
    B, V = K.shape[:2]
