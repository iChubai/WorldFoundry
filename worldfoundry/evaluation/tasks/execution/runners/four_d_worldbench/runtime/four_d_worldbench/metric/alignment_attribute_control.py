    ...
    for prompt_dict in tqdm(prompt_dict_ls):
        ...
        for video_path in video_paths:
            ...
            for i in range(len(base_question)):
                ...
                prompt_text = f"""Watch the video carefully and answer the following question about the specific attribute in the video:
...
# 提取生成的token（排除输入token）
                ...
                #question_detail["is_yes"] = "yes" in output_text.lower()
...
def compute_dynamic_attribute(json_dir, device, submodules_dict, **kwargs):  
    _, prompt_dict_ls = load_dimension_info(json_dir, dimension='alignment_attribute_control', lang='en')
    ...
    final_average_score = sum([d["video_results"] for d in video_details]) / len(video_details) if video_details else 0
