"""OpenAI-compatible inference backend for Intentional Change."""
...
from ..skills.skill_intentional_change_vlm import BACKEND_OUTPUT_SCHEMA


BACKEND_ID = "openai_compatible.intentional_change_vlm"
    ...
    def infer(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for url in urls:
            ...
            if attempts and attempts[-1]["status_code"] not in {404, 405}:
                break
        if response is None:
            raise RuntimeError(f"intentional VLM request failed: {last_error}")
