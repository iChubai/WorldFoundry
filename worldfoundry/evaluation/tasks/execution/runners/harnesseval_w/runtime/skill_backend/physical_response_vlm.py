"""OpenAI-compatible inference backend for Physical Response."""
...
from ..skills.skill_physical_response_vlm import BACKEND_OUTPUT_SCHEMA


BACKEND_ID = "openai_compatible.physical_response_vlm"
...
class OpenAICompatibleBackend:
    """Send prepared messages to a model service and return raw inference output."""
    ...
    def infer(self, messages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for url in urls:
            ...
            if attempts and attempts[-1]["status_code"] not in {404, 405}:
                break
        if response is None:
            raise RuntimeError(f"physical response VLM request failed: {last_error}")
