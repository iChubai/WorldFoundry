...
sys.modules.setdefault("vbench", sys.modules[__name__])
...
class VBench(object):
    ...
    def build_full_info_json(self, videos_path, name, dimension_list, prompt_list=[],  special_str='', verbose=False, custom_image_folder=None, mode='vbench_standard', **kwargs):
        ...
        if mode=='custom_input':
            ...
            prompts_root = Path(os.environ.get("WORLDFOUNDRY_VBENCH_PROMPTS_ROOT", str(Path(CUR_DIR).parent / "prompts")))
            category_dir = prompts_root / "prompts_per_category"
            category_supported = [Path(category).stem for category in os.listdir(category_dir)]
