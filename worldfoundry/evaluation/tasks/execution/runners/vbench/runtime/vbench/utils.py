...
from worldfoundry.base_models.capabilities import vbench_asset_path, vbench_dino_hub_config
try:
...
def init_submodules(dimension_list, local=False, read_frame=False):
    ...
    for dimension in dimension_list:
        ...
        if dimension == 'background_consistency':
            # read_frame = False
            vit_b_path = str(vbench_asset_path("vbench_clip_vit_b32_checkpoint")) if local else 'ViT-B/32'

            submodules_dict[dimension] = [vit_b_path, read_frame]
        elif dimension == 'human_action':
            umt_path = str(vbench_asset_path("vbench_umt_checkpoint"))
            ...
            submodules_dict[dimension] = {
                    'config': str(amt_config_path()),
                    'ckpt': str(amt_checkpoint_path())
                }
        ...
        # Assign the DINO model path for subject consistency dimension
            ...
            submodules_dict[dimension] = vbench_dino_hub_config(read_frame=read_frame)
        elif dimension == 'aesthetic_quality':
            aes_path = str(vbench_asset_path("vbench_aesthetic_linear_checkpoint"))
            vit_l_path = str(vbench_asset_path("vbench_clip_vit_l14_checkpoint")) if local else 'ViT-L/14'
            submodules_dict[dimension] = [vit_l_path, aes_path]
        elif dimension == 'imaging_quality':
            musiq_spaq_path = str(vbench_asset_path("vbench_musiq_spaq_checkpoint"))
            ...
            submodules_dict[dimension] = {
                "model_weight": str(vbench_asset_path("vbench_grit_checkpoint"))
            }
        elif dimension == 'scene':
            submodules_dict[dimension] = {
                "pretrained": str(vbench_asset_path("vbench_tag2text_checkpoint")),
                "image_size":384, 
                "vit":"swin_b"
            }
        elif dimension == 'appearance_style':
            if local:
                submodules_dict[dimension] = {"name": str(vbench_asset_path("vbench_clip_vit_b32_checkpoint"))}
            else:
                submodules_dict[dimension] = {"name": 'ViT-B/32'}
        elif dimension in ["temporal_style", "overall_consistency"]:
            submodules_dict[dimension] = {
                "pretrain": str(vbench_asset_path("vbench_viclip_checkpoint")),
            }
