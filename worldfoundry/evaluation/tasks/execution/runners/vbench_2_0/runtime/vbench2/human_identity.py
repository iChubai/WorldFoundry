...
from worldfoundry.base_models.perception_core.tracking.cotracker import Visualizer
from worldfoundry.base_models.perception_core.face.arcface import resnet_face18
from worldfoundry.base_models.perception_core.face.retinaface import load_retinaface_state_dict
...
def compute_human_identity(json_dir, device, submodules_dict, **kwargs):
    try:
        ...
        raise ModuleNotFoundError(
            "VBench-2.0 Human_Identity requires the optional `retinaface` package. "
            "Install the benchmark metric profile before running this dimension."
        ) from exc

    _, prompt_dict_ls = load_dimension_info(json_dir, dimension='human_identity', lang='en')
    
    retina_state_dict = load_retinaface_state_dict(submodules_dict["retina"], map_location="cpu")
