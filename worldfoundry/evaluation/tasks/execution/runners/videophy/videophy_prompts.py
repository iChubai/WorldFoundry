"""VideoPhy prompt materialization from bundled in-tree assets."""
...
def resolve_videophy_root(explicit: Path | None = None) -> Path | None:
    return first_existing_dir(
        explicit,
        resolve_env_path("WORLDFOUNDRY_VIDEOPHY_ROOT"),
...
def resolve_prompts_json_path(
    *,
    ...
    if explicit is not None:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"VideoPhy prompts JSON not found: {path}")
        return path
    env_manifest = resolve_env_path("WORLDFOUNDRY_VIDEOPHY_PROMPT_MANIFEST")
    if env_manifest is not None:
        if not env_manifest.is_file():
            raise FileNotFoundError(f"VideoPhy prompts JSON not found: {env_manifest}")
        return env_manifest
    bundled = bundled_benchmark_asset(BENCHMARK_ID, PROMPTS_JSON_REL)
    if bundled.is_file():
        return bundled
    root = repo_root or resolve_videophy_root()
    if root is None:
        raise FileNotFoundError(
            "VideoPhy prompts JSON is missing. Set WORLDFOUNDRY_VIDEOPHY_PROMPT_MANIFEST "
            "or provide bundled assets under worldfoundry/data/benchmarks/assets/videophy/."
        )
    candidate = root / PROMPTS_JSON_REL
    if not candidate.is_file():
        raise FileNotFoundError(f"VideoPhy prompts JSON not found: {candidate}")
    return candidate
...
def load_prompt_records(*, prompts_json_path: Path | None = None) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError(f"VideoPhy prompts JSON must be a list: {path}")
    records: list[dict[str, Any]] = []
    for index, row in enumerate(payload):
        if not isinstance(row, dict):
            continue
        prompt_id = str(row.get("prompt_id") or index + 1)
        caption = str(row.get("caption") or row.get("prompt") or "").strip()
        if not caption:
            continue
        records.append(
            {
    ...
    if not records:
        raise ValueError(f"VideoPhy prompt records are empty after validation: {path}")
    return records
...
def materialize_videophy_generation_requests(
    *,
    limit: int | None = None,
...
def copy_videophy_generated_videos(
    *,
