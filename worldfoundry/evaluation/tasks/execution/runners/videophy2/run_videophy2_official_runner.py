#!/usr/bin/env python3
"""Official runner for VideoPhy2 action-centric physical commonsense evaluation."""
...
from worldfoundry.evaluation.tasks.execution.runners.videophy2.videophy2_runtime import (
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run or normalize VideoPhy2 official outputs.")
    parser.add_argument("--benchmark-id", default="videophy2")
    ...
    parser.add_argument(
        "--run-official",
        action="store_true",
        help="Execute the in-tree VideoPhy2 AutoEval judge.",
    )
    parser.add_argument(
        "--run-fixture",
        action="store_true",
    ...
    parser.add_argument("--videophy2-root", type=Path)
    parser.add_argument("--prompt-manifest", type=Path)
    parser.add_argument("--autoeval-checkpoint", type=Path)
    parser.add_argument("--autoeval-lora-checkpoint", type=Path)
        ...
        rows.append(
            {
                ...
                "group": spec["group"],
                "source": "videophy2_official_runtime" if official_runtime_executed else "videophy2_results_file",
                "source_path": str(source_path),
                "components": components,
                "reason": None if score is not None else "score_not_available_in_videophy2_results",
            }
        )
    return rows
    ...
    *,
    ...
    return {
        ...
        "run": {
            ...
            "runner": "benchmark_zoo_videophy2_official_runner",
            "returncode": 0 if normalization_ok else 1,
            "judge_summary": dict(judge_summary or {}),
        },
        "benchmark": {"benchmark_id": benchmark_id, "name": "VideoPhy2"},
        "metrics": {
        ...
        "evaluation": {
            ...
            "kind": "videophy2_official_in_tree" if official_runtime_executed else "videophy2_result_normalizer",
        ...
        "notes": [
            "VideoPhy2 in-tree runtime materializes bundled prompts.json T2V requests and {prompt_id}.mp4 names.",
            "VideoPhy2 official execution uses the vendored base_models AutoEval runtime and WORLDFOUNDRY_VIDEOPHY2_AUTOEVAL_CKPT.",
        ],
...
def normalize_videophy2_results(
    ...
    *,
    ...
    official_results_path = args.official_results_path or resolve_env_path("WORLDFOUNDRY_VIDEOPHY2_RESULTS_PATH")
    if official_results_path is None:
        raise ValueError(
            "--official-results-path, WORLDFOUNDRY_VIDEOPHY2_RESULTS_PATH, or --run-official is required"
        )
    ...
    computed = compute_videophy2_metrics(rows=result_rows, results_path=Path(official_results_path))
    ...
    strict = args.strict or os.environ.get("WORLDFOUNDRY_VIDEOPHY2_STRICT", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    per_sample_rows = [
        {
            ...
            "pc": row.get("pc"),
            "joint": row.get("joint"),
            "physics_rules_followed": row.get("physics_rules_followed"),
            "physics_rules_unfollowed": row.get("physics_rules_unfollowed"),
        }
...
def run_official_videophy2(args: argparse.Namespace) -> dict[str, Any]:
    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    generated_dir = args.generated_artifact_dir or resolve_env_path("WORLDFOUNDRY_GENERATED_ARTIFACT_DIR")
    ...
    judge_summary = run_videophy2_judge(
        generated_artifact_dir=Path(generated_dir),
        output_dir=output_dir,
        config=judge_config_from_env(
            checkpoint=args.autoeval_checkpoint,
            lora_checkpoint=args.autoeval_lora_checkpoint,
    ...
    results_json = Path(str(judge_summary.get("results_json") or (output_dir / "videophy2_results.json")))
    ...
    return normalize_videophy2_results(
        normalize_args,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.run_fixture:
        sample_path = benchmark_task_sample_path("videophy2")
        if sample_path is None:
            print("error: no checked-in VideoPhy2 sample results found", file=sys.stderr)
            return 2
        args.official_results_path = sample_path
    try:
        if args.run_official:
            scorecard = run_official_videophy2(args)
        else:
            scorecard = normalize_videophy2_results(args)
        ...
        scorecard = {
            ...
            "benchmark": {"benchmark_id": args.benchmark_id, "name": "VideoPhy2"},
            "metrics": {
                "leaderboard": {},
                "per_metric": {},
            ...
            "evaluation": {
                "available": False,
                "kind": "videophy2_result_normalizer",
        ...
        if args.json:
            ...
            print(f"videophy2: failed ({exc})", file=sys.stderr)
        return 1
    ...
    if args.json:
        ...
        print(f"videophy2: {scorecard.get('evaluation', {}).get('kind', 'unknown')}")
