...
def compute_epe(flow1, flow2, crop=30, k=1.0, error_threshold=5.0):
    ...
    flow1_cropped = flow1[
        start_y : start_y + crop_size_H, start_x : start_x + crop_size_W, :
    ]
    flow2_cropped = flow2[
        start_y : start_y + crop_size_H, start_x : start_x + crop_size_W, :
    ]
    ...
    y_coords, x_coords = np.meshgrid(
        np.arange(crop_size_H), np.arange(crop_size_W), indexing="ij"
    )
...
class OpticalFlowAverageEndPointErrorMetric(BaseMetric):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            checkpoint_path = materialize_local_file(Path(checkpoint), kind="sea_raft")
            args.path = str(checkpoint_path)
            log_progress(
                "metric_load",
                metric="optical_flow_aepe",
                status="from_ckpt",
    ...
    def _compute_flow_batch(self, image1, image2):
        img1 = F.interpolate(
            image1,
        ...
        flow_np = flow_down.detach().float().cpu().numpy()
        if flow_np.ndim == 3:
            flow_np = flow_np[None]
        return [item.transpose(1, 2, 0) for item in flow_np]

    def _compute_flow(self, image1, image2):
        flows = self._compute_flow_batch(image1, image2)
        if len(flows) != 1:
            raise ValueError(f"expected a single flow field, got {len(flows)}")
        return flows[0]
    ...
    def _compute_scores(
        ...
        images = list(rendered_images)
        if len(images) < 2:
            raise ValueError("optical_flow_aepe requires at least two frames")
        pairs = list(zip(images[:-1], images[1:]))
        batch_size = max(1, int(getattr(self, "_batch_size", 1) or 1))
        scores = []
        with torch.no_grad():
            for start, end in pair_batch_slices(len(pairs), batch_size):
                chunk = pairs[start:end]
                image1 = torch.cat([self.load_image(left) for left, _ in chunk], dim=0)
                image2 = torch.cat([self.load_image(right) for _, right in chunk], dim=0)
                flow1_list = self._compute_flow_batch(image1, image2)
                flow2_list = self._compute_flow_batch(image2, image1)
                for flow1, flow2 in zip(flow1_list, flow2_list):
                    epe_score, _failure_mask = compute_epe(flow1, flow2)
