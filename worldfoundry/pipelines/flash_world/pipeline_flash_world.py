...
def _flash_world_operator_cls():
    """Flash world operator cls helper function."""
    from ...operators.flash_world_operator import FlashWorldOperator

    return FlashWorldOperator
...
def _flash_world_representation_cls():
    ...
    return FlashWorldRepresentation


def _export_gaussians(*args, **kwargs):
    """Export gaussians helper function."""
    from ...base_models.three_dimensions.point_clouds.flash_world.utils import (
        export_gaussians,
    )

    return export_gaussians(*args, **kwargs)


class FlashWorldPipeline(PipelineABC):
    ...
    def __init__(
        ...
        self.operator = operator or _flash_world_operator_cls()()
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        # Prefer local Wan model path if available under WORLDFOUNDRY_CKPT_DIR
        if "wan_model_path" not in kwargs:
            from worldfoundry.core.io.paths import checkpoint_root_path
            local_wan = checkpoint_root_path() / "Wan2.2-TI2V-5B-Diffusers"
            if local_wan.is_dir():
                kwargs["wan_model_path"] = str(local_wan)
            else:
                kwargs["wan_model_path"] = "Wan-AI/Wan2.2-TI2V-5B-Diffusers"

        # Prefer local FlashWorld checkpoint if available
        if model_path is None:
            from worldfoundry.core.io.paths import checkpoint_root_path
            local_flash = checkpoint_root_path() / "flash-world"
            if local_flash.is_dir():
                model_path = str(local_flash)

        representation_model = _flash_world_representation_cls().from_pretrained(
    ...
    def save_results(
        ...
        if save_spz:
            if importlib.util.find_spec("spz") is None:
                warnings.warn(
                    "FlashWorld SPZ export requested but the optional `spz` package is not installed; "
                    "saving PLY/video artifacts only.",
                    RuntimeWarning,
                )
                save_spz = False
                spz_path = None
        ...
        if save_video:
            ...
            if 'video_path' in results and os.path.exists(results['video_path']):
                ...
                write_video(video_frames, video_path, fps=fps)
                saved_paths['video_path'] = video_path
