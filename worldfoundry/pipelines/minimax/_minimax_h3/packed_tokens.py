"""MiniMax H3 video/audio token patchify helpers (ported from SGLang Apache-2.0)."""
...
def minimax_h3_patchify_video_latent(
    ...
    *,
    ...
    """Pack a video latent [B,C,T,H,W] into DiT token rows [B*t*h*w, C*pt*ph*pw]."""
...
def minimax_h3_unpatchify_video_tokens(
    ...
    *,
    ...
    """Unpack DiT video token rows into a latent [B,C,T,H,W]."""
    ...
    if int(rows.shape[-1]) != expected_dim:
        raise ValueError(
            f"video token dim {int(rows.shape[-1])} != patch volume * channel {expected_dim} "
            f"for latent_shape={list(latent_shape)}, patch_size={[pt, ph, pw]}"
        )
    rows_per_sample = t * h * w
    if int(rows.shape[0]) % rows_per_sample:
        raise ValueError(
            f"video rows {int(rows.shape[0])} must be divisible by t*h*w {rows_per_sample} "
            f"for latent_shape={list(latent_shape)}"
        )
...
def minimax_h3_unpack_audio_tokens(
    ...
    *,
    ...
    """Unpack DiT audio token rows into an audio VAE latent [C, latent_dim, T]."""

    _rank(rows, "audio token rows", 2)
    audio_t = int(audio_t)
    audio_channel = int(audio_channel)
    if audio_t <= 0 or audio_channel <= 0:
        raise ValueError(f"audio_t and audio_channel must be positive, got {audio_t=} {audio_channel=}")
    if int(rows.shape[0]) != audio_t:
        raise ValueError(f"audio rows {int(rows.shape[0])} != audio_t {audio_t}")
    if audio_t % audio_channel:
        raise ValueError(f"audio_t must be divisible by audio_channel, got {audio_t=} {audio_channel=}")
