"""MiniMax H3 task profiles (ported from SGLang Apache-2.0).

Data table driving request validation and plan resolution for the three v1
tasks (t2va / fl2va / ref2va). Keyframes bind target geometry; references
remain independent.
"""
...
from .constants import MINIMAX_H3_DEFAULT_BRANCHES
...
# ``fl2va`` is the single public task name for all keyframe variants:
# first-only, last-only, first+last. Keep the accepted semantic signatures
# centralized so validation and downstream sinks reject stale payloads.
    ...
    def rule_for(self, *, role: str, condition_type: str) -> MiniMaxH3ConditionRule:
        for rule in self.condition_rules:
            if rule.role == role and rule.condition_type == condition_type:
                return rule
        raise ValueError(
            f"task {self.task!r} does not allow condition role={role!r} type={condition_type!r}"
        )
...
def minimax_h3_task_profile(task: str) -> MiniMaxH3TaskProfile:
    ...
    if profile is None:
        raise ValueError(
            f"unknown minimax_h3 task {task!r}; supported: {sorted(MINIMAX_H3_TASK_PROFILES)}"
        )
    return profile
...
def _validate_profiles() -> None:
    for task, profile in MINIMAX_H3_TASK_PROFILES.items():
        ...
        if len(roles) > 1:
            raise ValueError(f"task {task}: condition roles must not mix, got {sorted(roles)}")
        ...
        if (
            ...
            raise ValueError(f"task {task}: min_condition_count exceeds max_condition_count")
        for rule in profile.condition_rules:
            if rule.material_chain not in MINIMAX_H3_CANONICAL_MATERIAL_CHAINS:
                raise ValueError(f"task {task}: unknown material chain {rule.material_chain!r}")
