...
from worldfoundry.base_models.three_dimensions.point_clouds.flash_world.autoencoder_kl_wan import AutoencoderKLWan
from worldfoundry.base_models.three_dimensions.point_clouds.flash_world.transformer_wan import WanTransformer3DModel
from worldfoundry.base_models.three_dimensions.point_clouds.flash_world.reconstruction_model import (
    WANDecoderPixelAligned3DGSReconstructionModel,
)
from worldfoundry.base_models.three_dimensions.point_clouds.flash_world.utils import (
    create_raymaps,
    normalize_cameras,
...
def _resolve_hf_snapshot_root(value: str, required_files: Tuple[str, ...] = ()) -> str:
    """Resolve a repo id, HF cache repo root, or regular local directory."""
    ...
    if path.is_dir():
        ...
        return str(path)
    if _is_probable_hf_repo_id(value):
        return snapshot_download(
            value,
            local_files_only=_env_flag("WORLDFOUNDRY_HF_LOCAL_FILES_ONLY", False),
        )
    return str(path)
...
class FlashWorldRepresentation(BaseRepresentation):
    ...
    def from_pretrained(
        ...
        **kwargs
        ...
        model_root = _resolve_hf_snapshot_root(pretrained_model_path, required_files=("model.ckpt",))
        if model_root != pretrained_model_path:
            print(f"Resolved FlashWorld weights to: {model_root}")
    ...
    def _load_generation_system(model_root: str, device: str, **kwargs) -> nn.Module:
        ...
        wan_model_path = _resolve_hf_snapshot_root(
            str(wan_model_path),
            required_files=("model_index.json", "vae/config.json", "transformer/config.json"),
        )
...
class GenerationSystem(nn.Module):
    ...
    def __init__(
        ...
        from worldfoundry.base_models.three_dimensions.point_clouds.flash_world.autoencoder_kl_wan import (
            WanCausalConv3d,
        )
        with torch.no_grad():
    ...
    def generate(self, cameras, n_frame, image=None, text="", image_index=0, image_height=480, image_width=704, video_path=None, video_fps=15):  
        ...
        vae_device = self.device if not self.offload_vae else torch.device("cpu")
        self.vae.to(vae_device)
        self.text_encoder.to(self.device if not self.offload_t5 else "cpu")
        self.transformer.to(self.device)
        self.recon_decoder.to(vae_device)
