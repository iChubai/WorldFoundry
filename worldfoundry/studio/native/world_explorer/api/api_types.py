...
def pad_or_trim_array(arr: np.ndarray | None, target_size: int) -> np.ndarray | None:
	"""Trim a batch or extend it by repeating its final item."""
	if arr is None:
		return None
	...
	reps = (target_size - len(arr), *([1] * (arr.ndim - 1)))
...
class RequestBase:
	...
	def intrinsics_matrix(self, for_resolutions: np.ndarray | None) -> np.ndarray:
		...
		format used by the native World Explorer."""
...
class SeedingRequest(RequestBase):
	"""
	Contains data required to seed a world model with initial data
...
class RequestState(Enum):
	"""Lifecycle state for an in-process inference request."""
