...
from .api_types import SeedingRequest, SeedingResult, InferenceRequest, InferenceResult, \
...
class WorldExplorerClient():
	def __init__(
		...
		self.model = model
		self.client_id = f"world-explorer-{os.getpid()}"
		...
		testbed.video_path = join(output_dir, "world_explorer_%Y-%m-%d_%H-%M-%S.mp4")

		# --- Read model metadata in-process. The native viewer and the model share
		# one Python runtime; no HTTP, serialization, or polling server is needed.
		...
		testbed.gen3c_info = f"Loaded {metadata['model_name']} in this native session"
		...
		if model_inference_res is not None:
			for supported_res in model_inference_res:
				...
				tlog.warning(f"Client inference resolution {self.inference_resolution} is not"
				             f" supported by the model, adopting resolution {r} instead.")
		...
		while testbed.frame():
			...
			if self.model_requires_seeding and self.seeding_pending and self.testbed.gen3c_seed_path:
				...
				if seed_req is not None:
					...
					# Start local seeding; model initialization can still take time.
					self.send_seeding_request(seed_req)
				self.seeding_pending = False

			# Give model coroutines a chance to run between native frames.
			...
			if state.state in (RequestState.FAILED, RequestState.COMPLETE):
				...
				def on_result_received(result: InferenceResult | None,
					...
					if error is not None:
						tlog.error(f"Inference {req_id} failed!\n{error}")
						self.testbed.gen3c_inference_info = f"Error: {error}"
					...
					for res_i in range(len(result)):
						...
						if self.testbed.gen3c_save_frames:
							...
							tlog.success(f"[+] Wrote inference result: {safe_res_id}")
					...
					if (pred_depths is not None) and (pred_depth_indices is not None):
						try:
							...
							for j, frame_idx in enumerate(indices.tolist()):
								...
								# Results stay as arrays in the local process, so no video
								# decode is required before adding their point clouds.
				...
				if self.testbed.gen3c_inference_progress < 0:
					...
					self.testbed.gen3c_inference_info = f"[{spinner}] Running local model..."
				pass
	...
	def request_frames(self, sync: bool = False) -> asyncio.Task | InferenceResult:
		...
		# TODO: enforce the model's `max_frames_per_request` with a clear error message.
		now = time.monotonic()
		...
		match self.testbed.gen3c_camera_source:
			...
			case ngp.Gen3cCameraSource.Viewpoint:
				...
				tlog.error("Not implemented: camera movement source: Viewpoint")
				return
			...
			case _:
				raise ValueError("Unsupported camera movement source:",
		...
		# Add any necessary padding to match the model's batch size.
		req.pad_to_frame_count(n_frames_padded)

		# Start local inference and add it to the pending request list.
	...
	def request_frame(self, req: InferenceRequest, sync: bool = False) -> asyncio.Task | InferenceResult:
		"""Run one model request directly in the event loop that owns the viewer."""
		task = self.model.request_inference(req)
		if sync:
			return task
	...
	def _get_inference_results(self, request_id: str, on_result_received: callable) -> asyncio.Task:
		async def _runner() -> None:
			...
			self.testbed.gen3c_inference_info = "Running local model..."
			try:
				while True:
					result = self.model.inference_result_or_none(request_id)
					if result is not None:
						on_result_received(result)
						return
					await asyncio.sleep(0.05)
			except Exception as error:
				on_result_received(None, error)
			finally:
	...
	def load_seeding_data(self, seeding_data_path: str, display: bool = True,
		...
		# Seed-from-image: crop in-memory to match the model aspect ratio.
		# (Avoids rewriting the user's file; `load_v2v_seeding_data()` already produced `req.images`.)
		if (not os.path.isdir(seeding_data_path)) and len(req) == 1:
			try:
				img = req.images[0]
		...
		if normalize_cameras:
			...
			# communicating with the model or saving output.

		if display and (req.depths is not None):
			# If depth is not available yet, display the frames after local seeding finishes.
	...
	def display_seeding_data(self, req: SeedingRequest, res: SeedingResult | None = None,
		...
		if res is not None:
			# Adopt extrinsics and intrinsics from the model, which might
			...
			if res.depths is not None:
				# TODO: model-estimated depth may have a completely different scale.
				depths = res.depths
				if res.depths.shape[1:] != images.shape[1:3]:
					# Depth prediction took place at a different resolution,
					...
					for i in range(len(req)):
						...
						# Assume the model already adjusted the intrinsics
	...
	def send_seeding_request(self, req: SeedingRequest, sync: bool = False) -> asyncio.Task | None:
		"""Seed the already-instantiated local model without an HTTP round trip."""
		depth_was_missing = (req.depths is None)

		async def _seed() -> None:
			self.testbed.gen3c_seeding_progress = 0.0
			try:
				response = await self.model.seed_model(req)
	...
	async def _revert_last_generation(self) -> None:
		"""Revert the local model and viewer to the state before the last generation."""
		...
		try:
			result = await self.model.revert_last_generation()
			if result.get("success"):
				tlog.success("Model reverted successfully.")
			else:
				tlog.error(f"Model revert failed: {result.get('message', 'unknown error')}")
				return
		except Exception as error:
			tlog.error(f"Model revert failed: {error}")
			return
	...
	def gui_callback(self, event: str) -> bool:
		match event:
			...
			case "revert_last_generation":
				asyncio.create_task(self._revert_last_generation())
				return True
