...
from .api_types import SeedingRequest
...
def load_gen3c_seeding_data(data_directory: str, max_frames: int | None = None,
	...
	The viewer and model run in one process, so load arrays once and pass them
	directly to the model without an encode/decode transport layer.
	"""
	...
	images = imageio.imread(join(data_directory, "rgb.mp4"))
	if images.ndim != 4 or images.shape[-1] not in (3, 4):
		raise ValueError(f"Expected RGB video frames, got {images.shape}")
	images = images[..., :3].astype(np.float32) / 255.0
	...
	if len(images) != n_img:
		raise ValueError(f"rgb.mp4 has {len(images)} frames but depth.npz has {n_img}")
	if max_frames is not None:
		indices = np.arange(0, n_img, max(frames_stride, 1))[:max_frames]
		images = images[indices]
		depths = depths[indices]
		masks = masks[indices]
		cameras_to_world = cameras_to_world[indices]
		focal_lengths = focal_lengths[indices]
		principal_points = principal_points[indices]
