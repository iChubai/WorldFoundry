...
void Testbed::imgui() {
		...
		if (!m_camera_path.keyframes.empty() && ImGui::CollapsingHeader("Video generation", ImGuiTreeNodeFlags_DefaultOpen)) {
			...
			if (ImGui::BeginPopupModal("Confirm Revert##gen3c", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
				ImGui::TextWrapped(
					...
					"the viewer and local model to the previous state.\n\n"
					"Are you sure?"
				);
				ImGui::Separator();
	...
	ImGui::Begin("World Explorer v" NGP_VERSION);
	...
	if (m_testbed_mode == ETestbedMode::Gen3c && ImGui::CollapsingHeader("World model generation", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		switch (m_gen3c_camera_source) {
			...
			default: throw std::runtime_error("Unsupported camera source.");
		}
	...
	if (ImGui::CollapsingHeader("Rendering", m_testbed_mode == ETestbedMode::Gen3c ? 0 : ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		std::string spp_string = fmt::format("({} spp)", std::max(render_buffer->spp(), 1u));
		ImGui::Text(
			": %.01fms for %dx%d %s",
			m_render_ms.ema_val(),
...
bool Testbed::keyboard_event() {
	...
	if (translate_vec != vec3(0.0f)) {
		...
		translate_camera(translate_vec, mat3(m_camera));
	}
...
void Testbed::blit_texture(
	...
	// Blit the floating-point render texture into the target framebuffer.
...
__global__ void reproject_kernel(
	...
	auto get_reprojected_dist = [&](float t) -> Result {
		...
		// This is used e.g. to exclude sky pixels by setting their depth to 0 in the Python viewer/model.
...
void Testbed::render(bool skip_rendering) {
	...
	{
		// Don't count the time spent allocating buffers and resetting render state as part of the frame time.
		...
		for (auto&& view : m_views) {
			...
			if (m_foveated_rendering) {
				if (m_dynamic_foveated_rendering) {
					...
					resolution_scale =
						clamp(resolution_scale, vec2(1.0f / m_foveated_rendering_max_scaling), vec2(1.0f));
...
void Testbed::init_window(int resw, int resh, bool hidden, bool second_window) {
	...
	std::string title = "World Explorer";
...
#	endif
	...
	if (gl_version_major < 3 || (gl_version_major == 3 && gl_version_minor < 1)) {
		throw std::runtime_error{
			fmt::format("Unsupported OpenGL version {}.{}. World Explorer requires at least OpenGL 3.1", gl_version_major, gl_version_minor)
		};
	}
	...
	// relative to the working directory. Instead, place imgui.ini in the World Explorer project directory.
...
__global__ void spherical_checkerboard_kernel(
	...
	// Blend with a checkerboard before compositing transparent pixels.
