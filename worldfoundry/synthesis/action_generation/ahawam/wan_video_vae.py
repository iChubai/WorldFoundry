...
from worldfoundry.base_models.diffusion_model.models.autoencoders.wan.model import Resample
...
from worldfoundry.base_models.diffusion_model.models.autoencoders.wan.model import Down_ResidualBlock
...
class WanVideoVAE(nn.Module):
    def __init__(self, z_dim=16):
        ...
        # These normalization constants are not checkpoint parameters.  Keep
        # them materialized while the module itself is built on ``meta``.
...
class WanVideoVAE38(WanVideoVAE):
    def __init__(self, z_dim=48, dim=160):
        ...
        # ``WanVideoVAE38`` is constructed inside the meta-device policy
        # context, but these normalization constants are not checkpoint
        # parameters.  Materialize them explicitly so strict ``assign=True``
        # restoration cannot leave the encode/decode scale on ``meta``.
        self.mean = torch.tensor(mean, device="cpu")
        self.std = torch.tensor(std, device="cpu")
        self.scale = [self.mean, 1.0 / self.std]
