...
from .action_base import ActionHead
from .targets import validate_inference_targets
...
def staged_hf_file(filename: str, repo_id: str = WAN_HF_REPO_ID) -> str:
    """Resolve one component from an hfd-staged Hub snapshot without networking."""

    directory = resolve_local_hf_model_path(repo_id, required_files=(filename,))
    return str(directory / filename)


def ensure_file(path: str | None, hf_filename: str, repo_id: str = WAN_HF_REPO_ID) -> str:
    """Return an explicit local file or its hfd-staged official counterpart."""
    if path is not None and os.path.exists(path):
        return path
    return staged_hf_file(hf_filename, repo_id)
...
class WANPolicyHeadConfig(PretrainedConfig):
    ...
    target_video_height: int | None = field(
        default=None, metadata={"help": "Target video height for resize (e.g. 160 for even latent with VAE38)."}
    )
    ...
    skip_component_loading: bool = field(
        default=False,
        metadata={"help": "Skip loading individual component weights (used when loading from full pretrained model)."},
    )

    hidden_size: int = field(default=1024, metadata={"help": "Input embedding dimension."})
    action_dim: int = field(default=None, metadata={"help": "Action dimension."})
    action_horizon: int = field(default=None, metadata={"help": "Action horizon."})
    # The video and action streams can follow different inference schedules.
    decouple_inference_noise: bool = field(
        default=False,
        metadata={"help": "Use decoupled noise schedules during inference (video stays noisy, action fully denoises)."},
    )
    ...
    defer_lora_injection: bool = field(
        default=False, metadata={"help": "Defer LoRA injection until after loading pretrained weights."}
    )
...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        validate_inference_targets(config.text_encoder_cfg, path="action_head.text_encoder_cfg")
        validate_inference_targets(config.image_encoder_cfg, path="action_head.image_encoder_cfg")
        validate_inference_targets(config.vae_cfg, path="action_head.vae_cfg")
        validate_inference_targets(config.diffusion_model_cfg, path="action_head.diffusion_model_cfg")
        ...
        self.text_encoder.load_state_dict(load_state_dict(text_enc_path, device="cpu"))
        ...
        self.image_encoder.model.load_state_dict(
            load_state_dict(img_enc_path, device="cpu"),
            strict=False,
        )
        ...
        self.vae.model.load_state_dict(load_state_dict(vae_path, device="cpu"))
    ...
    def set_frozen_modules_to_eval_mode(self):
        """Keep every checkpoint component in deterministic inference mode."""
        self.model.eval()
    ...
    def encode_image(self, image, num_frames, height, width):
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
            ...
            image_zeros = torch.zeros(
                batch_size, 3, num_frames - 1, height, width, dtype=torch.bfloat16, device=self._device
            )
    ...
    def add_lora_to_model(
        ...
        """Materialize the adapter modules required by LoRA checkpoints."""
    ...
    def _exchange_predictions(
        ...
        recv_ops = [
            dist.P2POp(
                op=dist.irecv, tensor=other_pred, group_peer=(self.ip_rank + 1) % self.ip_size, group=self.ip_group
            )
    ...
    def lazy_joint_video_action(
        self, backbone_output: BatchFeature, action_input: BatchFeature, latent_video: torch.Tensor | None = None
    ) -> BatchFeature:
        ...
        # Wan 5B checkpoints require their configured latent-grid resolution.
        ...
        scheduler_time = (
            total_time - kv_creation_time - diffusion_time - text_encoder_time - image_encoder_time - vae_time
        )
    ...
    def post_initialize(self):
        ...
        if LOAD_TRT_ENGINE is not None:
            ...
            from .. import tensorrt as trt_utils

            model_path = LOAD_TRT_ENGINE
            model_type = "ar_5B" if self.model.num_heads == 24 else "ar_14B"
            self.trt_engine = trt_utils.load_tensorrt_engine(
                model_path,
