"""PaliGemma 2 expert backbone used by GigaBrain-0 inference."""
...
class Gemma2AttentionWithExpert(nn.Module):
    ...
    def forward(
        ...
        key_states = apply_rotary_embedding(key_states, cos, sin)
        ...
        att_output = scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class PaliGemma2WithExpertModel(nn.Module):
    ...
    def tie_weights(self):
        ...
        if isinstance(output_module, nn.Linear) and isinstance(input_emb, nn.Embedding) and output_module.weight.shape == input_emb.weight.shape:
            ...
            # (e.g., when loading a compatible checkpoint with a resized vocabulary).
            with torch.no_grad():
