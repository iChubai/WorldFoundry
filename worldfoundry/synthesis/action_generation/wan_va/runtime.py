...
from .serving import run_async_server_mode


WAN_VA_CHECKPOINT_ENV = "WORLDFOUNDRY_WAN_VA_CHECKPOINT_DIR"


def _resolve_checkpoint_dir(args, config):
    checkpoint_dir = args.checkpoint_dir or os.getenv(WAN_VA_CHECKPOINT_ENV)
    if checkpoint_dir is None:
        checkpoint_dir = getattr(config, "wan22_pretrained_model_name_or_path", None)
    if not checkpoint_dir:
        raise ValueError(
            "Wan-VA requires a local checkpoint directory. Pass --checkpoint-dir "
            f"or set {WAN_VA_CHECKPOINT_ENV}."
        )
    resolved = resolve_worldfoundry_path(checkpoint_dir).resolve()
    if not resolved.is_dir():
        raise FileNotFoundError(f"Wan-VA local checkpoint directory does not exist: {resolved}")
    return str(resolved)
...
class VA_Server:
    ...
    def infer(self, obs):
        ...
        if reset:
            ...
            return dict()
        if not hasattr(self, "frame_st_id"):
            self._reset(prompt=prompt)
        if compute_kv_cache:
            logger.info(
                f"################# Compute KV Cache #################")
...
def run(args):
    ...
    config.param_dtype = resolve_inference_dtype(
        torch.device(f"cuda:{local_rank}"),
        getattr(config, "param_dtype_request", config.param_dtype),
    )
    config.rank = rank
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Local LingBot-VA checkpoint directory containing transformer, vae, tokenizer, and text_encoder.",
    )
