...
class EvokeTransformer3DModel(
    ...
    def __init__(
        ...
        is_amplify_history: bool = False,
        history_scale_mode: str = "per_head",  # [scalar, per_head]
        # Metadata flag for raw sink frame support.
        ...
        # Metadata flag: pipeline uses this to decide whether to provide sink_latents / nearby_sink_latents.
    ...
    def forward(
        ...
        # Raw sink frame inputs (only used when use_raw_sink_frames=True).
        sink_latents=None,           # [B, 16, 1, H_lat, W_lat] first sink frame at history idx=0
        nearby_sink_latents=None,    # [B, 16, K, H_lat, W_lat] nearby sink frames (last K memory frames)
        ...
        # 2b. Prepend nearby sink tokens before target; final order: [first_sink | long | mid | short | nearby_sink | target].
        if nearby_sink_latents is not None:
            assert indices_latents_history_short is not None, (
                "nearby_sink_latents needs indices_latents_history_short for the frame index"
            )
            nearby_sink_lat = nearby_sink_latents.to(hidden_states)
            nearby_emb = self.patch_embedding(nearby_sink_lat)
            ...
            rotary_emb = torch.cat([nearby_rope, rotary_emb], dim=1)
        ...
        # 5b. Prepend first sink token (history idx=0) at the very front.
        if sink_latents is not None:
            sink_lat = sink_latents.to(hidden_states)
            sink_emb = self.patch_embedding(sink_lat)
            ...
            rotary_emb = torch.cat([sink_rope, rotary_emb], dim=1)
