...
# Patch model norms with Triton flash implementations.
...
def layernorm_forward(
    ...
    # Cast to float32 for numerical stability (matches FP32LayerNorm).
    X_row = tl.load(X + col_offsets, mask=mask, other=0).to(tl.float32)
    W_row = tl.load(W + col_offsets, mask=mask, other=0).to(tl.float32)
    b_row = tl.load(b + col_offsets, mask=mask, other=0).to(tl.float32)

    mean_X = tl.sum(X_row, axis=0) / n_cols
    XX = tl.where(mask, X_row - mean_X, 0)  # mask out padding before variance sum
...
def layernorm_backward(
    ...
    # Follows karpathy/llm.c layernorm backward derivation.
    row_idx = tl.program_id(0)
    col_offsets = tl.arange(0, BLOCK_SIZE)
    mask = col_offsets < n_cols
...
class Flash_Layernorm(torch.autograd.Function):
    ...
    def forward(ctx, X, W, b, eps):
        ...
        ctx.W_requires_grad = W.requires_grad
    ...
    def backward(ctx, dY):
        ...
        # Compute dW/db before the kernel, which overwrites dY in-place with dX.
        dW = None
        db = None
        if ctx.W_requires_grad or ctx.b_requires_grad:
            inv_var = r.unsqueeze(1).to(torch.float32)
            mean = mu.unsqueeze(1).to(torch.float32)
            normed = (X.to(torch.float32) - mean) * inv_var
            dY32 = dY.to(torch.float32)
            if ctx.W_requires_grad:
                dW = (dY32 * normed).sum(dim=0).to(W.dtype)
            if ctx.b_requires_grad:
                db = dY32.sum(dim=0).to(b.dtype)
...
def _rms_layernorm_backward(
    ...
    inv_var = tl.load(r).to(tl.float32)  # saved inverse std from forward
    normed = X_row * inv_var
...
def _gemma_rms_layernorm_forward(
    ...
    # Gemma RMSNorm variant: uses (W+1) scaling, fully in float32.
    row_idx = tl.program_id(0)
    col_offsets = tl.arange(0, BLOCK_SIZE)
    mask = col_offsets < n_cols
