...
def replace_rope_with_flash_rope():
    from ...diffusers_version import transformer_evoke_diffusers
    from .. import transformer_evoke

    transformer_evoke_diffusers.apply_rotary_emb_transposed = apply_rotary_emb_transposed_flash
    transformer_evoke.apply_rotary_emb_transposed = apply_rotary_emb_transposed_flash
...
class Flash_RoPE_Transposed(torch.autograd.Function):
    ...
    def forward(ctx, x, freqs_cis):
        ...
        # seq_len==0 occurs in t2v mode (no history tokens); reshape(0,-1) is ambiguous → early return.
        if seq_len == 0:
            ctx.save_for_backward(torch.empty(0, device=x.device), torch.empty(0, device=x.device))
            ctx.n_heads = n_heads
            ctx.BLOCK_SIZE = 1
            ctx.num_warps = 1
