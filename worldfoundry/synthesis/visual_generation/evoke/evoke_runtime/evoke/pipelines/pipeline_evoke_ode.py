...
from ..utils.inference_noise import add_noise, convert_flow_pred_to_x0
...
class EvokePipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def stage2_sample(
        ...
        latents_history_long: torch.Tensor = None,
        attention_kwargs: Optional[dict] = None,
        # Camera poses -> per-stage cam_plucker_emb; None means plucker-less, as in _flow_loss.
        cam_Ks: Optional[torch.Tensor] = None,
        cam_c2ws: Optional[torch.Tensor] = None,
        cam_base_h: Optional[int] = None,
        cam_base_w: Optional[int] = None,
        ...
        for i_s in range(stage2_num_stages):
            ...
            # cam_plucker_emb for this stage: single resolution, shared by every step of it.
            _cam_plk = None
            if cam_Ks is not None and cam_c2ws is not None:
                from evoke.modules.camera_control import prepare_cam_plucker_emb
                _cam_plk = prepare_cam_plucker_emb(
                    cam_Ks.to(device, dtype=torch.float32),
                    cam_c2ws.to(device, dtype=torch.float32),
                    int(latents.shape[-2]) * self.vae_scale_factor_spatial,
                    int(latents.shape[-1]) * self.vae_scale_factor_spatial,
                    cam_base_h,
                    cam_base_w,
