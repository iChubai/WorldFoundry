...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        model_root = _wan_model_root("Wan2.1-T2V-1.3B")
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=str(model_root / "google" / "umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=str(_wan_model_root("Wan2.1-T2V-1.3B") / "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def forward(
        ...
        updating_cache: Optional[bool] = None,
    ) -> torch.Tensor:
        ...
        if kv_cache is not None:
            cache_options = {}
            if updating_cache is not None:
                cache_options["updating_cache"] = updating_cache
