...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
from worldfoundry.base_models.diffusion_model.models.autoencoders.hunyuan_video.causal3d import (
    HunyuanVideoCausal3DAutoencoder as AutoencoderKLCausal3D,
)
from worldfoundry.base_models.diffusion_model.models.networks.hunyuan_video.gamecraft import HYVideoDiffusionTransformer
from worldfoundry.base_models.diffusion_model.schedulers.flow_dpm import retrieve_timesteps
from worldfoundry.base_models.diffusion_model.schedulers.lvdm.schedule_math import rescale_noise_cfg
...
class HunyuanVideoGamePipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    Pipeline for text-to-video generation using HunyuanVideo (Game Version).

    Uses shared encoders, VAE and scheduling with GameCraft context conditioning.
    """
    ...
    def __init__(
        ...
        self.set_progress_bar_config(**(progress_bar_config or {}))
        ...
        **kwargs,
                ...
                if prompt_mask[0].sum() > 575:
                    prompt_mask[0] = torch.cat(
                        [
                            ...
                            torch.zeros((1, prompt_mask.shape[1] - prompt_mask[0].sum() + 575)).to(prompt_mask),
                        ],
                        dim=1,
                    )
        ...
        if frame_length == 10:
            ...
            mask_concat = torch.ones(
                gt_latents.shape[0], 1, gt_latents.shape[2], gt_latents.shape[3], gt_latents.shape[4]
            ).to(device=gt_latents.device)
            mask_concat[:, :, 1:, ...] = 0.0
        else:
            gt_latents_concat[:, :, gt_latents_concat.shape[2] // 2 :, :, :] = 0.0
            mask_zeros = torch.zeros(
                gt_latents.shape[0], 1, gt_latents.shape[2] // 2, gt_latents.shape[3], gt_latents.shape[4]
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                cam_latents_ = (
                    torch.cat([uncond_cam_latents, cam_latents], dim=0)
                    if self.do_classifier_free_guidance
                    else cam_latents
                )
                ...
                if ip_cfg_scale > 0:
                    ...
                    noise_pred = (
                        ...
                        + start_scale * (noise_pred_ip - noise_pred_text)
                    )
        ...
        if return_latents:
            return (
                HunyuanVideoPipelineOutput(videos=image),
                latents,
                timesteps,
                last_latents,
                last_latents[:, :, -1:, ...],
