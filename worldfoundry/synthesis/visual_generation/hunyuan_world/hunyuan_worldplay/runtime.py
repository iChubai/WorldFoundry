...
from worldfoundry.synthesis.visual_generation.hunyuan_world.hunyuan_worldplay.utils.retrieval_context import (
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        runtime = cls(
            ...
            flow_shift=pipeline.config.flow_shift,
            guidance_scale=pipeline.config.guidance_scale,
            embedded_guidance_scale=pipeline.config.embedded_guidance_scale,
            progress_bar_config=pipeline._progress_bar_config,
            vision_num_semantic_tokens=pipeline.config.vision_num_semantic_tokens,
            vision_states_dim=pipeline.config.vision_states_dim,
            glyph_byT5_v2=pipeline.config.glyph_byT5_v2,
            byt5_model=pipeline.byt5_model,
            byt5_tokenizer=pipeline.byt5_tokenizer,
            byt5_max_length=pipeline.byt5_max_length,
            prompt_format=pipeline.prompt_format,
            execution_device=pipeline.execution_device,
...
class _HunyuanWorldPlayInternalPipeline(HunyuanVideo15Resources, DiffusionPipeline):
    ...
    def _process_single_byt5_prompt(self, prompt_text, device):
        ...
        if len(glyph_texts) > 0:
            if not self._byt5_available:
                raise RuntimeError(
                    "Prompt contains quoted glyph text, but Glyph-SDXL-v2 is not "
                    "available. Expected text_encoder/Glyph-SDXL-v2 with "
    ...
    def create_pipeline(
        ...
        device=None,
        action_ckpt=None,
        allow_uninitialized_action: bool = False,
        **kwargs,
    ):
...
def load_runtime(*args, **kwargs):
    return HunyuanWorldPlayRuntime.from_pretrained(*args, **kwargs)
