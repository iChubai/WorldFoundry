...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, cam_emb, t_mod, freqs, grid_shape=None):
        ...
        # encode camera
        cam_emb = cam_emb.to(dtype=self.cam_encoder.weight.dtype, device=self.cam_encoder.weight.device)
        cam_emb = self.cam_encoder(cam_emb).to(dtype=input_x.dtype, device=input_x.device)
        if grid_shape is None:
            if input_x.shape[1] % cam_emb.shape[1] != 0:
                raise ValueError(
                    f"Cannot align camera embeddings of length {cam_emb.shape[1]} "
                    f"with token sequence length {input_x.shape[1]}."
                )
            cam_emb = cam_emb.repeat_interleave(input_x.shape[1] // cam_emb.shape[1], dim=1)
        else:
            f, h, w = grid_shape
            if cam_emb.shape[1] != f:
                repeats = (f + cam_emb.shape[1] - 1) // cam_emb.shape[1]
                cam_emb = cam_emb.repeat(1, repeats, 1)[:, :f]
            cam_emb = cam_emb.unsqueeze(2).repeat(1, 1, h * w, 1)
            cam_emb = rearrange(cam_emb, 'b f s d -> b (f s) d')
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        if has_image_input:
            self.img_emb = MLPProj(1280, dim)  # clip_feature_dim = 1280
                ...
                if use_gradient_checkpointing_offload:
                    with torch.autograd.graph.save_on_cpu():
                        x = torch.utils.checkpoint.checkpoint(
                            ...
                            x, context, cam_emb, t_mod, freqs, (f, h, w),
                            use_reentrant=False,
                        )
                else:
                    x = torch.utils.checkpoint.checkpoint(
