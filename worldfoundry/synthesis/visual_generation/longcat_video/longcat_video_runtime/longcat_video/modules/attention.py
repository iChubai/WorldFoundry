...
class MultiHeadCrossAttention(nn.Module):
    ...
    def _process_cross_attn(self, x, cond, kv_seqlen):
        ...
        def _sdpa_varlen():
            ...
            for batch_index, key_length in enumerate(kv_seqlen):
                key_length = int(key_length)
                q_i = q_flat[batch_index * N:(batch_index + 1) * N].transpose(0, 1).unsqueeze(0)
                k_i = k_flat[k_start:k_start + key_length].transpose(0, 1).unsqueeze(0)
                v_i = v_flat[k_start:k_start + key_length].transpose(0, 1).unsqueeze(0)
                out_i = F.scaled_dot_product_attention(q_i, k_i, v_i, dropout_p=0.0, scale=scale)
