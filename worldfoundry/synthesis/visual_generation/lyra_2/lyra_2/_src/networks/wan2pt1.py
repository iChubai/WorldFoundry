...
try:
    from transformer_engine.pytorch.attention import DotProductAttention as _TransformerEngineDotProductAttention
except ImportError:
...
class _SDPADotProductAttention(nn.Module):
    def __init__(
        self,
        num_attention_heads,
        kv_channels,
        ...
        **kwargs,
        ...
        self.num_gqa_groups = num_gqa_groups or num_attention_heads
    ...
    def forward(self, q, k, v, *args, **kwargs):
        if self.qkv_format != "bshd":
            raise NotImplementedError(f"Unsupported qkv_format={self.qkv_format!r}")
        q = q.transpose(1, 2)
        k = k.transpose(1, 2)
        v = v.transpose(1, 2)
...
DotProductAttention = _TransformerEngineDotProductAttention or _SDPADotProductAttention
...
def rope_apply(x, video_size: VideoSize, freqs):
    """
    Apply rotary embedding with plain PyTorch ops.
...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, video_size: VideoSize, freqs, kq_bias=None):
        r"""
        Args:
            ...
            kq_bias(Tensor|None): Optional bias added to input before Q and K projections only (not V).
        """
