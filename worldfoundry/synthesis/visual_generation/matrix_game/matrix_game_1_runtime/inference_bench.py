...
class VideoGenerator:
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        """
        self.args = args
        self.device = getattr(args, "selected_cuda_device", None) or torch.device(
    ...
    def _init_models(self) -> None:
        ...
        dit.eval()
        self.i2v_type = self.args.i2v_type
        if self.i2v_type is None:
            self.i2v_type = "refiner"
        
        # Initialize text encoder
        textenc_path = self.args.textenc_path
        weight_dtype = torch.bfloat16 if self.args.bfloat16 else torch.float32
        self.text_enc = get_text_enc('matrixgame', textenc_path, weight_dtype=weight_dtype, i2v_type=self.i2v_type)
    ...
    def _process_condition(self, condition: Dict, image_path: str) -> None:
        ...
        keyboard_condition = torch.tensor(
            condition['keyboard_condition'][:self.video_length], dtype=torch.float32
        ).unsqueeze(0)
        mouse_condition = torch.tensor(
            condition['mouse_condition'][:self.video_length], dtype=torch.float32
        ).unsqueeze(0)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--i2v_type", choices=("concat", "refiner"), default=None,
                        help="I2V conditioning type. Defaults to DiT config inference.")
...
def _select_cuda_device(gpu_id: str | None) -> torch.device:
    """Select the requested CUDA device before model construction."""

    if not torch.cuda.is_available():
        return torch.device("cpu")
    requested = str(gpu_id or "").split(",", 1)[0].strip()
    if requested:
        try:
            requested_index = int(requested)
        except ValueError as exc:
            raise ValueError(f"Invalid --gpu_id value: {gpu_id!r}") from exc
        visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES")
        device_count = torch.cuda.device_count()
        if visible_devices and requested_index >= device_count:
            requested_index = 0
...
def main():
    ...
    args = parse_args()
    args.selected_cuda_device = _select_cuda_device(args.gpu_id)
