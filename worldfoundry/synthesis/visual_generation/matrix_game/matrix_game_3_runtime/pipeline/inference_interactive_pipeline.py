...
def _matrix_game3_asset_root() -> str:
    env_root = os.environ.get("WORLDFOUNDRY_MATRIX_GAME3_ASSET_ROOT")
    if env_root:
        return env_root
    worldfoundry_root = Path(RUNTIME_ROOT).resolve().parents[3]
    return str(worldfoundry_root / "data" / "models" / "runtime" / "configs" / "matrix_game_3" / "assets")


MOUSE_ICON_PATH = os.path.join(_matrix_game3_asset_root(), "images", "mouse.png")
                            ...
                            self.vae_latent_queue.put(
                                {
                                    ...
                                    "clip_idx": clip_idx,
                                    "mouse_condition": mouse_condition_curr.detach().cpu(),
                                    "keyboard_condition": keyboard_condition_curr.detach().cpu(),
                                    "mouse_condition_all": mouse_condition_all.detach().cpu(),
                                    "keyboard_condition_all": keyboard_condition_all.detach().cpu(),
                                    "interactive": True,
                                    "save_name": save_name,
                                }
                            )
