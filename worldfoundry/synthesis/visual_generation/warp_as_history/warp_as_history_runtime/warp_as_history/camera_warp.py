...
def default_pi3_repo() -> Path:
    return PI3_SOURCE_ROOT


def default_pi3x_ckpt() -> Path:
    override = os.environ.get("WAH_PI3X_CKPT_PATH")
    if override:
...
def _load_pi3x_model(Pi3X: Any, device: torch.device):
    ...
    if not ckpt.is_file():
        raise FileNotFoundError(
            ...
            "checkpoints/pi3x/model.safetensors before running inference or training."
        )
