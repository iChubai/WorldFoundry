...
class PluckerEmbeddingChannelStack(nn.Module):
    def __init__(self, output_dim: int, t_ratio: int = 4):
        super().__init__()
        
        plucker_dim = 6
        stacked_dim = plucker_dim * t_ratio
        self.t_ratio = t_ratio

        self.mlp = nn.Sequential(
            nn.Linear(stacked_dim, output_dim * 2),
            nn.GELU(),
            nn.Linear(output_dim * 2, output_dim)
        )

    def forward(self, plucker_embedding: torch.Tensor) -> torch.Tensor:

        B, F, H, W, P = plucker_embedding.shape

        plucker_embedding = rearrange(plucker_embedding, 'b (f g) h w p -> b f g h w p', g=self.t_ratio)
        plucker_embedding = rearrange(plucker_embedding, 'b f g h w p -> b f h w (g p)')

        plucker_embedding = rearrange(plucker_embedding, 'b f h w c -> b (f h w) c')
        
        final_output = self.mlp(plucker_embedding)
...
class DiTBlock(nn.Module):
    def __init__(self, has_image_input: bool, dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6):
        ...
        self.plucker_embedder = PluckerEmbeddingChannelStack(output_dim=dim)
    ...
    def forward(self, x, context, t_mod, freqs, plucker_grid):
        # When training with progressive noise the time modulation is per-frame, so
        # `t_mod` has an extra sequence axis (`b f 6 d` instead of `b 6 d`).
        ...
        if has_seq:
            ...
            # Broadcast each per-frame modulation across all spatial tokens of that frame.
            shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
                repeat(u, 'b f d -> b (f s) d', s=x.shape[1] // u.shape[1]) for u in
                [shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp])
        ...
        if plucker_grid is not None:
            plucker_features = self.plucker_embedder(plucker_grid)
            plucker_dim = plucker_features.shape[-2]
            x[:, -plucker_dim:, :] = x[:, -plucker_dim:, :] + plucker_features
...
class WanModel(torch.nn.Module):
    ...
    def convert_to_plucker(self, extrinsics, intrinsics, height, width, num_frames, original_video_wh, batch_size, device):
    ...
    def forward(self,
        ...
        if f > 16: # with long-term memory
            ...
            self.freqs[0][:f].view(f, 1, 1, -1).expand(f, h, w, -1),
            self.freqs[1][:h].view(1, h, 1, -1).expand(f, h, w, -1),
            self.freqs[2][:w].view(1, 1, w, -1).expand(f, h, w, -1)], dim=-1).reshape(f * h * w, 1, -1).to(x.device)
        

        plucker_grid = self.convert_to_plucker(
